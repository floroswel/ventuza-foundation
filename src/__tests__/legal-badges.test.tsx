...
describe("/legal/badges", () => {
  ...
  it("randează toate badge-urile din registry (label RO + criteriu RO)", () => {
    for (const b of Object.values(BADGES)) {
      ...
      expect(html, `lipsește criteriu pentru ${b.code}`).toContain(b.criteria.ro);
    }
  });
  ...
  it("afișează condiția de expirare (RO) în HTML pentru fiecare badge care expiră", () => {
  ...
  it("nu afișează 'Expirare:' pentru badge-urile permanente", () => {
    ...
    for (const b of permanent) {
      expect(html).not.toContain(`Expirare: ${b.criteria.ro}`);
    }
  });
});
