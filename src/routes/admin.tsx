...
function AdminDashboard() {
      ...
      <div className="flex min-h-dvh items-center justify-center px-6 text-center">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Ai nevoie de rol admin pentru această pagină.
          </p>
          <button
            onClick={() => navigate({ to: "/" })}
  ...
  const allItems: (NavItem & { id: Section; adminOnly?: boolean })[] = [
    ...
    },
    {
      ...
      icon: Bot,
      group: "Operations",
      hint: "Asistent procedural",
    },
    { id: "broadcast", label: "Broadcast", icon: Send, group: "Operations", adminOnly: true },
    {
      id: "broadcast2",
      label: "Broadcast v2 · targeting",
      icon: Send,
      group: "Operations",
      adminOnly: true,
      hint: "Filtre reale + dry-run + campanii",
    },
    {
      id: "support",
      label: "Ticketing",
    ...
    },
    {
      ...
      icon: FileText,
      group: "Operations",
      hint: "Canned replies + scor satisfacție",
    },
    {
      id: "appeals",
      label: "Contestații DSA",
      icon: Gavel,
      group: "Operations",
      hint: "Art. 20 DSA",
    },
    {
      id: "userops",
      label: "User ops · view-as",
      icon: Eye,
      group: "Operations",
      adminOnly: true,
      hint: "Impersonare read-only + sesiuni",
    },
    {
      id: "intel",
      label: "Intelligence · MRR/Cohorts",
      icon: TrendingUp,
      group: "Operations",
      hint: "MRR/ARR, retention, funnel",
    },
    {
      id: "killswitch",
      label: "Kill switches",
    ...
    {
      id: "riskqueue",
      label: "Risc · review nou",
      icon: Flag,
      group: "Trust & Safety",
      hint: "Conturi noi auto-flag pentru review",
    },
    {
      id: "riskdash",
      label: "Risc · dashboard",
      icon: Activity,
      group: "Trust & Safety",
      hint: "Distribuție, trend, semnale live",
    },
    ...
    {
      id: "legalp0",
      label: "Legal P0 · NCMEC/DSA/Country",
      icon: ShieldAlert,
      group: "Compliance",
      adminOnly: true,
      hint: "CSAM+NCMEC, breach 72h, DSA SoR, country-risk, e-Factura",
    },
    { id: "policies", label: "Politici", icon: FileText, group: "Compliance" },
    {
      id: "policyengine",
      label: "Policy Engine · reguli/shadow",
      icon: Scale,
      group: "Compliance",
      adminOnly: true,
      hint: "Rules-as-code, versionare, backtest, shadow mode",
    },
    { id: "audit", label: "Audit", icon: ScrollText, group: "Compliance" },
    {
      id: "legaldocs",
      label: "Documente legale · CMS",
      icon: FileText,
      group: "Compliance",
      adminOnly: true,
      hint: "Editează live Termeni, Privacy, DSA etc.",
    },
    ...
    { id: "demoseed", label: "Demo seed", icon: Sparkles, group: "System", adminOnly: true },
    {
      ...
      group: "System",
      adminOnly: true,
      hint: "app_settings + feature_flags",
    },
    {
      id: "staff",
      label: "Staff & roluri",
    ...
    },
    {
      ...
      group: "System",
      adminOnly: true,
      hint: "Redactare mesaj, edit partner, storno factură",
    },
  ];
...
function AdminPanelError({
  ...
  const forbidden = /forbidden|denied|insufficient|not allowed|rol|role|policy|permission/i.test(
    error,
  );
  return (
...
function OverviewPanel() {
  ...
  const attention = [
    ...
    { label: "B2B pending", count: m.bizPending ?? 0, tone: "warn" as const },
    {
      label: "SOS 7 zile",
      count: a.sos7d ?? 0,
      tone: (a.sos7d ?? 0) > 0 ? "danger" : ("neutral" as const),
    },
  ];
            ...
            <GlassCard padding="p-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <Kpi
                  label="Ads pending"
                  value={m.adsPending}
                  tone={m.adsPending > 0 ? "warn" : "default"}
                />
                <Kpi
                  label="B2B pending"
                  value={m.bizPending}
                  tone={m.bizPending > 0 ? "warn" : "default"}
                />
            ...
            <GlassCard padding="p-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <Kpi
                  label="SOS 7 zile"
                  value={a.sos7d}
                  tone={a.sos7d > 0 ? "danger" : "default"}
                  hint={a.sos7d > 0 ? "verifică urgent" : undefined}
                />
                <Kpi
                  label="Rapoarte pending"
                  value={m.reportsPending}
                  tone={m.reportsPending > 0 ? "warn" : "default"}
                />
              </div>
            </GlassCard>
          </div>
          ...
          <GlassCard padding="p-4" elevated>
            ...
            <ul className="space-y-2">
              {attention.map((a) => (
                <li
                  key={a.label}
                  className="flex items-center justify-between gap-2 rounded-lg border border-[var(--admin-border)]/60 bg-[var(--admin-surface-2)] px-3 py-2"
                >
                  <span className="text-xs text-[var(--admin-text-dim)]">{a.label}</span>
                  <StatusBadge
                    tone={
                      a.tone === "danger" ? "rejected" : a.tone === "warn" ? "pending" : "neutral"
                    }
                  >
...
function UsersPanel({ meId }: { meId: string }) {
  return <EnterpriseUsersPanel meId={meId} />;
}
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <input
          ...
          placeholder="coloană (ex: display_name)"
        ...
        <input
          ...
          placeholder="caută (ilike)"
          className="flex-1 min-w-[180px] rounded-full border border-border bg-surface px-3 py-2 text-xs"
        />
        <button
          onClick={() => {
            setPage(0);
          ...
          disabled={busy}
          className="rounded-full bg-primary px-3 py-2 text-xs text-primary-foreground disabled:opacity-50"
        >
          Caută
        </button>
        <button
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id ?? JSON.stringify(r).slice(0, 50)} className="border-t border-border">
                {columns.slice(0, 6).map((c) => (
                  <td key={c} className="max-w-[200px] truncate px-2 py-2">
                    {formatCell(r[c])}
                  </td>
                ))}
                <td className="px-2 py-2 text-right">
                  <button
                    ...
                    className="mr-1 rounded-full bg-primary/10 px-2 py-1 text-[10px] text-primary"
                  >
                    edit
                  </button>
                  {r.id && (
                    <button
                      onClick={() => del(r.id)}
                      className="rounded-full bg-red-500/15 px-2 py-1 text-[10px] text-red-500"
                    >
      ...
      <div className="flex items-center justify-end gap-2">
        ...
        <button
          disabled={(page + 1) * limit >= count || busy}
          onClick={() => setPage((p) => p + 1)}
          className="rounded-full border border-border px-3 py-1.5 text-xs disabled:opacity-30"
        >
      ...
      {editing && (
        <Modal
          ...
          title={`Editare rând ${String(editing.id).slice(0, 8)}`}
        >
          <textarea
            value={editing.json}
...
function ReportsPanel({ meId }: { meId: string }) {
  ...
  const suspend = async (userId: string, hours: number, reason: string) => {
    setBusy(true);
    const { error } = await supabase.rpc("moderator_suspend_user", {
      _target: userId,
      _hours: hours,
      _reason: reason,
    });
    ...
    toast.success(hours > 0 ? `Suspendat ${hours}h` : "Anulat");
    load();
  };
  const ban = async (userId: string) => {
    const reason = prompt("Motiv ban:");
    if (!reason) return;
  ...
  if (reports.length === 0)
    return (
      <AdminPanelEmpty label="🎉 Nimic de moderat (empty legitim — nicio raportare în așteptare)." />
    );
  ...
  const columns: Column<Report>[] = [
    ...
    },
    {
      ...
      cell: (r) => (
        <MonoNumber
          value={r.reported_profile?.report_count ?? 0}
          className={(r.reported_profile?.report_count ?? 0) >= 3 ? "text-[var(--admin-warn)]" : ""}
        />
      ),
    ...
    },
    {
      ...
      priority: 3,
      cell: (r) => (
        <span className="line-clamp-2 max-w-[280px] text-[11px] text-[var(--admin-text-dim)]">
          {r.details ?? "—"}
        </span>
      ),
    ...
    },
    {
      ...
      priority: 2,
      cell: (r) => (
        <span className="admin-mono text-[10px] text-[var(--admin-text-dim)]">
    ...
    },
    {
      ...
      cell: (r) => (
        <div className="flex flex-wrap justify-end gap-1" onClick={(e) => e.stopPropagation()}>
          <button
            disabled={busy}
            onClick={() => suspend(r.reported_id, 24, r.reason)}
            className="rounded-md bg-[var(--admin-warn-soft)] px-2 py-1 text-[10px] text-[var(--admin-warn)]"
          >
            <Ban className="inline size-3" />
            24h
          </button>
          <button
            disabled={busy}
            onClick={() => suspend(r.reported_id, 168, r.reason)}
            className="rounded-md bg-[var(--admin-danger-soft)] px-2 py-1 text-[10px] text-[var(--admin-danger)]"
          >
            <Ban className="inline size-3" />
            7z
          </button>
          <button
            disabled={busy}
            onClick={() => ban(r.reported_id)}
            className="rounded-md bg-[var(--admin-danger-soft)] px-2 py-1 text-[10px] font-semibold text-[var(--admin-danger)]"
          >
            Ban
          </button>
          <button
            ...
            className="rounded-md bg-[var(--admin-success-soft)] px-2 py-1 text-[10px] text-[var(--admin-success)]"
          >
...
function RiskPanel() {
  ...
  const verify = async (uid: string) => {
    setBusy(true);
    const { error } = await supabase.rpc("moderator_verify_user", { _target: uid });
    setBusy(false);
    if (error) return toast.error(error.message);
  ...
  const warn = async (uid: string) => {
    const reason = prompt("Motiv:");
    if (!reason) return;
    setBusy(true);
    const { error } = await supabase.rpc("moderator_warn_user", { _target: uid, _reason: reason });
    setBusy(false);
    if (error) return toast.error(error.message);
    toast.success("Avertizat");
    load();
  };
  const suspend = async (uid: string, hours: number) => {
    setBusy(true);
    const { error } = await supabase.rpc("moderator_suspend_user", {
      _target: uid,
      _hours: hours,
  ...
  const ban = async (uid: string) => {
    ...
    if (!confirm("Ban permanent?")) return;
    setBusy(true);
    const { error } = await supabase.rpc("moderator_ban_user", { _target: uid, _reason: reason });
    setBusy(false);
    if (error) return toast.error(error.message);
    toast.success("Banat");
    load();
  ...
  if (risk.length === 0)
    return <AdminPanelEmpty label="✅ Niciun risc detectat (empty legitim)." />;

  const riskTone = (n: number): "rejected" | "pending" | "info" =>
    n >= 80 ? "rejected" : n >= 60 ? "pending" : "info";
  const cols: Column<RiskRow>[] = [
    {
      ...
      align: "right",
      cell: (r) => (
        <StatusBadge tone={riskTone(r.risk_score)}>
          <MonoNumber value={r.risk_score} />
        </StatusBadge>
      ),
      ...
      cell: (r) => (
        <div className="flex flex-wrap gap-1">
          {r.duplicate_photo_accounts > 0 && (
            <StatusBadge tone="rejected">poză dup ×{r.duplicate_photo_accounts}</StatusBadge>
          )}
    ...
    },
    {
      ...
      priority: 3,
      cell: (r) => (
        <span className="text-[10px] uppercase text-[var(--admin-text-faint)]">
          {r.recent_flag_kinds.join(" · ") || "—"}
        </span>
      ),
    },
    {
      ...
      cell: (r) => (
        <div className="flex flex-wrap justify-end gap-1" onClick={(e) => e.stopPropagation()}>
          <button
            disabled={busy}
            onClick={() => verify(r.user_id)}
            className="rounded-md bg-[var(--admin-accent-soft)] px-2 py-1 text-[10px] text-[var(--admin-accent)]"
          >
            <ShieldCheck className="inline size-3" />
          </button>
          <button
            disabled={busy}
            onClick={() => warn(r.user_id)}
            className="rounded-md bg-[var(--admin-warn-soft)] px-2 py-1 text-[10px] text-[var(--admin-warn)]"
          >
  ...
  return (
    <DataTable
      ...
      rowKey={(r) => r.user_id}
      exportName="risc"
      searchPlaceholder="Filtrare risc…"
    />
  );
}
...
function AdsPanel() {
  ...
  const load = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from("ad_campaigns")
        .select(
          "id, title, body, placement, status, city, cta_url, image_url, starts_at, ends_at, budget_cents, impressions, clicks, advertiser_id",
        )
        .order("created_at", { ascending: false })
        .limit(200);
      if (error) throw error;
  ...
  if (ads.length === 0)
    return (
      <AdminPanelEmpty label="Nicio campanie publicitară (empty legitim — niciun advertiser nu a publicat încă)." />
    );
  ...
  const cols: Column<any>[] = [
    ...
    },
    {
      ...
      cell: (a) => (
        <div className="flex items-center gap-2">
          {a.image_url && (
            <img src={a.image_url} alt="" className="size-8 shrink-0 rounded object-cover" />
          )}
          <div className="min-w-0">
            <div className="truncate font-medium">{a.title}</div>
            <div className="truncate text-[10px] text-[var(--admin-text-faint)]">
              {a.brand_name ?? a.advertiser_id?.slice(0, 8)}
            </div>
          </div>
        </div>
      ),
    ...
    },
    {
      ...
      header: "Plasare",
      priority: 2,
      cell: (a) => <StatusBadge tone="info">{a.placement}</StatusBadge>,
    ...
    },
    {
      ...
      header: "Oraș",
      priority: 3,
      cell: (a) => <span className="text-xs text-[var(--admin-text-dim)]">{a.city ?? "—"}</span>,
      sortValue: (a) => a.city ?? "",
    },
    {
      key: "budget",
      ...
      align: "right",
      priority: 2,
      cell: (a) => <MonoNumber value={(a.budget_cents / 100).toFixed(0)} suffix=" RON" />,
      sortValue: (a) => a.budget_cents ?? 0,
    },
    {
      key: "imp",
      header: "Imp",
      align: "right",
      priority: 3,
      cell: (a) => <MonoNumber value={a.impressions} />,
    ...
    },
    {
      ...
      align: "right",
      priority: 3,
      cell: (a) => <MonoNumber value={a.clicks} />,
  ...
  return (
    <DataTable
      ...
      rowKey={(a) => a.id}
      exportName="ads"
      searchPlaceholder="Filtrare ads…"
    />
  );
}
...
function BizPanel() {
  ...
  const setStatus = async (id: string, status: "approved" | "rejected" | "reviewing") => {
    ...
    setBusy(false);
    if (error) return toast.error(error.message);
    toast.success(status === "approved" ? "Aprobat — rol business acordat" : status);
    load();
  };
  ...
  if (apps.length === 0)
    return <AdminPanelEmpty label="Nicio cerere B2B în așteptare (empty legitim)." />;
  ...
  const cols: Column<any>[] = [
    {
      ...
      header: "Status",
      cell: (a) => (
        <StatusBadge tone={a.status === "reviewing" ? "info" : "pending"}>{a.status}</StatusBadge>
      ),
      sortValue: (a) => a.status,
    },
    {
      ...
      cell: (a) => (
        <div className="min-w-0">
          ...
          <div className="truncate text-[10px] text-[var(--admin-text-faint)]">
            {a.legal_name} · CUI {a.cui ?? "—"}
          </div>
        </div>
      ),
      sortValue: (a) => a.brand_name ?? a.legal_name ?? "",
      searchValue: (a) =>
        `${a.brand_name ?? ""} ${a.legal_name ?? ""} ${a.cui ?? ""} ${a.contact_email ?? ""} ${a.city ?? ""}`,
    },
    {
      key: "entity",
      header: "Tip",
      priority: 2,
      cell: (a) => <StatusBadge tone="neutral">{a.entity_type}</StatusBadge>,
    ...
    },
    {
      ...
      priority: 3,
      cell: (a) => (
        <span className="text-xs text-[var(--admin-text-dim)]">{a.category ?? "—"}</span>
      ),
      ...
      cell: (a) => (
        <div className="min-w-0">
          ...
          <a
            ...
            className="admin-mono block truncate text-[10px] text-[var(--admin-accent)] hover:underline"
          >
            {a.contact_email}
          </a>
        </div>
      ...
      cell: (a) => (
        <div className="flex flex-wrap justify-end gap-1" onClick={(e) => e.stopPropagation()}>
          <button
            ...
            className="rounded-md bg-[var(--admin-success-soft)] px-2 py-1 text-[10px] text-[var(--admin-success)]"
          >
            <Check className="inline size-3" /> Aprobă
          </button>
          <button
            disabled={busy}
            onClick={() => setStatus(a.id, "reviewing")}
            className="rounded-md bg-blue-500/15 px-2 py-1 text-[10px] text-blue-300"
          >
            Review
          </button>
          <button
  ...
  return (
    <DataTable
      ...
      rowKey={(a) => a.id}
      exportName="b2b"
      searchPlaceholder="Filtrare cereri B2B…"
    />
  );
}
