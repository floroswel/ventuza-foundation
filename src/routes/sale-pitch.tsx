...
const STACK = [
  ...
  {
    k: "Backend",
    v: "Supabase (Postgres 15 + RLS + pgcrypto), TanStack server fns pe Cloudflare Workers",
  },
...
const FEATURES = {
  Dating: [
    ...
    "Mesaje 1:1 cu media + voice",
    "Stories",
    "Visitors & favorites",
    "Block bilateral enforced la DB",
    "Travel mode",
    "PIN lock + discreet mode",
    "SOS / panic tools",
  ],
  "B2B Partners": [
    "Portal partener /partner",
    "Wizard creare venues/events/offers",
    "Quota & moderare obligatorie",
    "Facturare RO completă (OP bancar, TVA, numerotare neîntreruptă)",
    "Grace period + retrogradare automată",
    "Notificări status partener",
    "Nearby cu prioritizare per plan",
  ],
  Admin: [
    "RBAC pe 6 roluri",
    "Audit log append-only",
    "Break-glass cu justificare",
    "2FA enforced",
    "Data masking by default",
    "CSAM no-render + hash blocklist",
    "DSA reports anonime",
    "Command palette (Cmd+K) + AI copilot",
  ],
  GDPR: [
    "Art. 9 minimizat (orientare opt-in, HIV eliminat complet)",
    "Consent registry centralizat",
    "Cascade withdrawal automat",
    "Registru Art. 30 versionat",
    "Subprocesatori documentați",
    "Right to be Forgotten complet",
    "DPIA pentru date sensibile",
  ],
};

const PRICE_RANGES = [
  {
    label: 'Asset sale „as-is"',
    range: "25.000 – 60.000 €",
    desc: "Fără founder, fără brand consacrat. Cumpărător oportunist preia codul + infra.",
  },
  {
  ...
  },
  {
    ...
    desc: "Founder rămâne să opereze. Player european care intră pe piața RO.",
  },
  {
    label: "Strategic sale (cu PMF)",
    range: "250.000 – 500.000+ €",
    desc: "Necesită D30 retention >20% și ≥1 partener B2B plătit. Grindr/Romeo/Hornet sau fond local.",
  },
];
...
function Pill({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 text-[10px] font-medium px-2 py-0.5 rounded-full border ${t}`}
    >
      {children}
    </span>
  );
      ...
      <main className="mx-auto max-w-4xl px-5 py-8 space-y-10 print:py-4 print:space-y-6">
        ...
        <section>
          <h2 className="text-lg font-semibold mb-3 flex items-center gap-2">
            <Lock className="size-4" /> Tech stack
          </h2>
        ...
        <section>
          <h2 className="text-lg font-semibold mb-3 flex items-center gap-2">
            <Globe2 className="size-4" /> Compliance posture
          </h2>
          <div className="rounded-xl border border-border bg-surface p-4 text-sm space-y-2">
            <p>
              · <strong>Registru Art. 30</strong> versionat în{" "}
              <code>docs/gdpr-art-30-register.md</code> + pagina{" "}
              <code>/legal/records-of-processing</code>.
            </p>
            <p>
              · <strong>Inventar procesatori</strong> public la <code>/legal/subprocessors</code>,
              sincronizat cu fluxurile reale.
            </p>
            <p>
              · <strong>Date Art. 9 criptate</strong> la nivel coloană cu pgcrypto. Cheie izolată în
              secret server. Decriptare doar prin RPC SECURITY DEFINER, grant strict pe
              service_role.
            </p>
            <p>
              · <strong>Triggere DB</strong> blochează scrieri pe câmpuri sensibile fără
            ...
            <p>
              ...
              hash perceptual + sha256 pentru blocare globală.
            </p>
            <p>
              · <strong>DSA reports anonime</strong>: <code>reporter_email/user_id</code> strippate
              din toate proiecțiile admin.
            </p>
            <p>
              · <strong>Documente legale P0 complete</strong>: terms, privacy, cookies, community,
              safety, age-policy, dmca, dsa, subprocessors.
            </p>
          </div>
        </section>
        ...
        <section>
          ...
          <div className="space-y-2">
            {PRICE_RANGES.map((p) => (
              <div
                key={p.label}
                className="rounded-xl border border-border bg-surface p-4 grid md:grid-cols-[1fr_180px] gap-3 items-center"
              >
                <div>
                  <div className="text-sm font-semibold">{p.label}</div>
                  <div className="text-xs text-muted-foreground mt-1">{p.desc}</div>
                </div>
                <div className="text-right font-mono text-base font-bold text-primary">
                  {p.range}
                </div>
              </div>
            ))}
          ...
          <p className="mt-3 text-xs text-muted-foreground">
            <strong>Asking price recomandat azi (1.000 useri instalați, MRR=0):</strong>{" "}
            <span className="text-primary font-semibold">65.000 – 90.000 €</span> asset sale +
            handover 3 luni. La 3-5 parteneri B2B plătitori (~3.000 € MRR) intervalul urcă natural
            la 150.000 – 200.000 €.
          </p>
        </section>
        ...
        <section>
          ...
          <div className="rounded-xl border border-border bg-surface p-4 text-sm space-y-2">
            <p>
              · Echipă: 1 senior full-stack + 1 backend + 0.5 designer + 0.5 consultant GDPR ·{" "}
              <strong>8–12 luni</strong>
            </p>
            <p>
              · Manoperă la rate RO de piață: <strong>150.000 – 250.000 €</strong>
            </p>
            <p>
              · Consultanță GDPR specializată Art. 9: <strong>15.000 – 30.000 €</strong>
            </p>
            <p>
              · Cost biblioteci, infra dev, testing: <strong>5.000 – 10.000 €</strong>
            </p>
            <p className="pt-2 border-t border-border">
              <strong>Total replacement cost: 170.000 – 290.000 €</strong>
            </p>
          </div>
        </section>
        ...
        <section>
          ...
          <ol className="space-y-2">
            {DAY_30.map((s, i) => (
              <li
                key={s}
                className="rounded-xl border border-border bg-surface p-3 flex gap-3 text-sm"
              >
                <span className="inline-flex size-6 shrink-0 items-center justify-center rounded-full bg-primary/15 text-primary text-xs font-bold">
                  {i + 1}
                </span>
                <span>{s}</span>
              </li>
        ...
        <footer className="text-center text-[10px] text-muted-foreground pt-4 border-t border-border">
          Document confidențial. Cifrele de preț sunt estimări de piață RO/EU 2026, nu garanții.
          Evaluarea finală depinde de due diligence (DAU/MAU, retention, MRR B2B, contracte
          semnate).
        </footer>
      </main>
    </div>
