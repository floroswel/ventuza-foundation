  ...
  return (
    <main className="mx-auto flex min-h-dvh max-w-md flex-col bg-background pb-24">
      <header className="sticky top-0 z-20 flex items-center gap-3 border-b border-border/60 bg-background/85 px-3 py-3 backdrop-blur">
        <Link to="/discover" className="rounded-full p-2 text-muted-foreground hover:text-foreground">
            ...
            {notifications.map((n) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <div className={`flex size-9 shrink-0 items-center justify-center rounded-full ${n.read_at ? "bg-muted text-muted-foreground" : "bg-primary/15 text-primary"}`}>
                    ...
                    {!n.read_at && <span className="mt-2 size-2 shrink-0 rounded-full bg-primary" />}
                  </Wrapper>
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
