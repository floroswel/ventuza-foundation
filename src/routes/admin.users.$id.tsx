...
import {
  ChevronLeft, Mail, ShieldAlert, KeyRound, LogOut, Pencil, Send, Trash2,
  RotateCcw, CheckCircle2, XCircle, Ban, AlertTriangle, Loader2, Crown,
} from "lucide-react";
...
import {
  adminGetUserActivity, adminUpdateUserProfile, adminChangeUserEmail,
  adminPushUnicast,
} from "@/lib/admin-user360.functions";
import {
  adminForceLogout, adminTriggerPasswordReset, adminManualAgeVerify,
  adminCancelDeletion,
...
export const Route = createFileRoute("/admin/users/$id")({
  head: () => ({ meta: [{ title: "User 360 — Admin Ventuza" }, { name: "robots", content: "noindex" }] }),
  ...
  return (
    <div className="container max-w-6xl py-6 space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <Link to="/admin" className="text-sm text-muted-foreground hover:underline inline-flex items-center gap-1">
        ...
        <div className="flex items-start justify-between flex-wrap gap-3">
          <div>
            <h1 className="text-2xl font-bold flex items-center gap-2">
              ...
              {profile.is_premium && <Crown className="h-5 w-5 text-amber-500" aria-label="Premium" />}
              {profile.verified && <CheckCircle2 className="h-5 w-5 text-emerald-500" aria-label="Vârstă verificată" />}
            </h1>
            ...
            <div className="flex flex-wrap gap-2 mt-3">
              {profile.banned_at && <Badge variant="destructive">Ban ({fmtDate(profile.banned_at)})</Badge>}
          ...
          <ActionsBar userId={id} profile={profile} authEmail={activity?.auth?.email ?? null}
            onSuccess={() => { viewQ.refetch(); actQ.refetch(); }} />
        </div>
      </Card>
      ...
      <Tabs defaultValue="overview">
        ...
        <TabsContent value="overview" className="space-y-3 mt-4">
          <Card className="p-4">
            ...
            <p className="mt-3 text-xs text-muted-foreground">
              Câmpurile Art. 9 (orientare, sănătate), locația precisă și conținutul mesajelor
              sunt accesibile EXCLUSIV prin <b>Break-glass</b> (Admin → Compliance).
            </p>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="auth" className="space-y-3 mt-4">
          ...
          <Card className="p-4">
            ...
            <MiniTable rows={activity?.devices ?? []} cols={[
          ...
          <Card className="p-4">
            ...
            <MiniTable rows={activity?.push_subscriptions ?? []} cols={[
              { k: "kind", h: "Tip" }, { k: "platform", h: "Platformă" },
        ...
        <TabsContent value="consents" className="space-y-3 mt-4">
          <Card className="p-4">
            <div className="text-sm font-semibold mb-2">Istoric consimțăminte (50 cele mai recente)</div>
            <MiniTable rows={view.consents} cols={[
              { k: "kind", h: "Kind" }, { k: "version", h: "Versiune" },
        ...
        <TabsContent value="reports" className="space-y-3 mt-4">
          <Card className="p-4">
            ...
            <MiniTable rows={activity?.reports_received ?? view.reports} cols={[
              { k: "reason", h: "Motiv" }, { k: "status", h: "Status" },
              { k: "reporter_id", h: "Reporter", fmt: (v) => v ? String(v).slice(0, 8) + "…" : "—" },
              { k: "created_at", h: "Data", fmt: fmtDate },
            ]} />
          </Card>
          <Card className="p-4">
            <div className="text-sm font-semibold mb-2">Rapoarte făcute de user</div>
            <MiniTable rows={activity?.reports_made ?? []} cols={[
              { k: "reason", h: "Motiv" }, { k: "status", h: "Status" },
              { k: "reported_id", h: "Țintă", fmt: (v) => v ? String(v).slice(0, 8) + "…" : "—" },
              { k: "created_at", h: "Data", fmt: fmtDate },
            ]} />
          </Card>
        </TabsContent>
        ...
        <TabsContent value="payments" className="space-y-3 mt-4">
          <Card className="p-4">
            ...
            <MiniTable rows={activity?.subscriptions ?? []} cols={[
              { k: "status", h: "Status" }, { k: "platform", h: "Platformă" },
          ...
          <Card className="p-4">
            <div className="text-sm font-semibold mb-2">Facturi parteneri (dacă userul e partener)</div>
            <MiniTable rows={activity?.invoices_as_partner ?? []} cols={[
              { k: "series", h: "Serie",
                fmt: (_, r: any) => `${r.series}-${String(r.number).padStart(5, "0")}/${r.year}` },
              { k: "status", h: "Status" },
              { k: "total_minor", h: "Total",
        ...
        <TabsContent value="risk" className="space-y-3 mt-4">
          <Card className="p-4">
            ...
            <MiniTable rows={activity?.risk_flags ?? []} cols={[
              { k: "kind", h: "Kind" }, { k: "severity", h: "Severitate" },
              { k: "status", h: "Status" },
        ...
        <TabsContent value="gdpr" className="space-y-3 mt-4">
          <Card className="p-4">
            ...
            <MiniTable rows={activity?.deletion_requests ?? []} cols={[
              { k: "status", h: "Status" },
            ...
            {(activity?.deletion_requests ?? []).some((d: any) => d.status === "scheduled") && (
              <RestoreDeletionButton
                requestId={(activity!.deletion_requests as any[]).find((d) => d.status === "scheduled").id}
                onDone={() => { actQ.refetch(); viewQ.refetch(); }}
              />
            )}
          </Card>
          <Card className="p-4">
            <div className="text-sm font-semibold mb-2">Verificări vârstă (Didit + override manual)</div>
            <MiniTable rows={view.verifications} cols={[
              { k: "provider", h: "Sursă" }, { k: "result", h: "Rezultat" },
...
function ActionsBar({
  userId, profile, authEmail, onSuccess,
}: { userId: string; profile: any; authEmail: string | null; onSuccess: () => void }) {
  return (
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <div className="space-y-3">
          {fields}
          <div>
            <Label htmlFor="reason" className="text-sm">Justificare (obligatorie, min. {minLen} caractere) <span className="text-destructive">*</span></Label>
            <Textarea id="reason" value={reason} onChange={(e) => setReason(e.target.value)}
              placeholder="Ex: cerere user pe email, ticket #1234, Art. 16 GDPR rectificare"
              rows={3} className="mt-1" />
            <p className="text-[11px] text-muted-foreground mt-1">
              Această justificare e salvată în <code>admin_audit_log</code> (append-only). Vor fi vizibile pentru auditor/super_admin.
            </p>
          </div>
        </div>
        <DialogFooter>
...
function EditProfileDialog({ userId, profile, onDone }: { userId: string; profile: any; onDone: () => void }) {
  ...
  const m = useMutation({ mutationFn: (j: string) => fn({ data: {
    userId,
    changes: {
  ...
  return (
    <ReasonDialog
      trigger={<Button variant="outline" size="sm"><Pencil className="h-4 w-4 mr-1" /> Editează profil</Button>}
      ...
      onConfirm={async (j) => { await m.mutateAsync(j); onDone(); }}
      fields={
        <div className="space-y-2">
          ...
          <div><Label>Birthdate (YYYY-MM-DD)</Label><Input type="date" value={birth} onChange={(e) => setBirth(e.target.value)} /></div>
          <div><Label>Travel city</Label><Input value={city} onChange={(e) => setCity(e.target.value)} maxLength={80} /></div>
        </div>
      }
    />
  );
...
function ChangeEmailDialog({ userId, currentEmail, onDone }: { userId: string; currentEmail: string | null; onDone: () => void }) {
  const [newEmail, setNewEmail] = useState("");
  const fn = useServerFn(adminChangeUserEmail);
  const m = useMutation({ mutationFn: (j: string) => fn({ data: { userId, newEmail, justification: j } }) });
  return (
    <ReasonDialog
      trigger={<Button variant="outline" size="sm"><Mail className="h-4 w-4 mr-1" /> Schimbă email</Button>}
      ...
      destructive
      onConfirm={async (j) => { await m.mutateAsync(j); onDone(); }}
      fields={<div><Label>Email nou</Label><Input type="email" value={newEmail} onChange={(e) => setNewEmail(e.target.value)} required /></div>}
    />
  );
}
...
function PushUnicastDialog({ userId, onDone }: { userId: string; onDone: () => void }) {
  ...
  const m = useMutation({ mutationFn: (j: string) => fn({ data: {
    userId, title, body, url: url || undefined, justification: j,
  } }) });
  return (
    <ReasonDialog
      trigger={<Button variant="outline" size="sm"><Send className="h-4 w-4 mr-1" /> Trimite mesaj</Button>}
      ...
      fields={
        <div className="space-y-2">
          <div><Label>Titlu (max 80)</Label><Input value={title} onChange={(e) => setTitle(e.target.value)} maxLength={80} /></div>
          <div><Label>Mesaj (max 280)</Label><Textarea value={body} onChange={(e) => setBody(e.target.value)} maxLength={280} rows={3} /></div>
          <div><Label>Link (opțional)</Label><Input value={url} onChange={(e) => setUrl(e.target.value)} placeholder="/settings sau https://…" maxLength={500} /></div>
        </div>
      }
    />
  );
...
function ForceLogoutDialog({ userId, onDone }: { userId: string; onDone: () => void }) {
  ...
  return (
    <ReasonDialog
      trigger={<Button variant="outline" size="sm"><LogOut className="h-4 w-4 mr-1" /> Force logout</Button>}
...
function PasswordResetDialog({ userId, onDone }: { userId: string; onDone: () => void }) {
  ...
  return (
    <ReasonDialog
      trigger={<Button variant="outline" size="sm"><KeyRound className="h-4 w-4 mr-1" /> Reset parolă</Button>}
...
function ManualAgeVerifyDialog({ userId, verified, onDone }: { userId: string; verified: boolean; onDone: () => void }) {
  const [makeVerified, setMakeVerified] = useState<boolean>(!verified);
  const fn = useServerFn(adminManualAgeVerify);
  const m = useMutation({ mutationFn: (j: string) => fn({ data: { userId, verified: makeVerified, justification: j } }) });
  return (
    <ReasonDialog
      trigger={<Button variant="outline" size="sm">
        {verified ? <XCircle className="h-4 w-4 mr-1" /> : <CheckCircle2 className="h-4 w-4 mr-1" />}
        Override vârstă
      </Button>}
      ...
      fields={
        <div className="flex items-center gap-3 text-sm">
          <label className="flex items-center gap-2">
            <input type="radio" checked={makeVerified} onChange={() => setMakeVerified(true)} /> Aprobă (verified=true)
          </label>
          <label className="flex items-center gap-2">
            <input type="radio" checked={!makeVerified} onChange={() => setMakeVerified(false)} /> Respinge (verified=false)
          </label>
        </div>
      }
    />
...
function SuspendDialog({ userId, onDone }: { userId: string; onDone: () => void }) {
  ...
  return (
    <ReasonDialog
      ...
      fields={
        <div><Label>Ore</Label>
          <Input type="number" min={1} max={720} value={hours} onChange={(e) => setHours(parseInt(e.target.value, 10) || 24)} />
        </div>
      }
    />
  );
...
function RestoreDeletionButton({ requestId, onDone }: { requestId: string; onDone: () => void }) {
  ...
  const m = useMutation({ mutationFn: (j: string) => fn({ data: { requestId, justification: j } }) });
  return (
    <div className="mt-3">
      <ReasonDialog
        trigger={<Button size="sm"><RotateCcw className="h-4 w-4 mr-1" /> Restore cont (în fereastra de 30 zile)</Button>}
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="text-left text-xs uppercase text-muted-foreground">
            {cols.map((c) => <th key={c.k} className="px-2 py-1 font-medium">{c.h}</th>)}
          </tr>
        </thead>
        <tbody>
          {rows.map((r, i) => (
...
function fmtDate(v: any) {
  if (!v) return "—";
  try { return new Date(v).toLocaleString("ro-RO", { dateStyle: "short", timeStyle: "short" }); }
