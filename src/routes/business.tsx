...
const schema = z
  .object({
    ...
    goals: z
      .string()
      .trim()
      .min(20, "Descrie pe scurt ce vrei să faci (min. 20 caractere)")
      .max(2000),
    ...
    accepts_lgbt_charter: z.literal(true, {
      errorMap: () => ({ message: "Trebuie să accepți charta LGBTQ+" }),
    }),
  })
  .superRefine((v, ctx) => {
...
function BusinessPage() {
  ...
  async function fetchAnaf() {
    if (!form.cui) {
      toast.error("Introdu CUI-ul mai întâi");
      return;
    }
    setAnafLoading(true);
    try {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      const payload = {
        ...form,
        monthly_budget_eur:
          form.monthly_budget_eur === "" ? undefined : Number(form.monthly_budget_eur),
      };
      ...
      if (res?.id) {
        try {
          localStorage.setItem(STATUS_KEY, JSON.stringify({ id: res.id, at: Date.now() }));
        } catch {
          /**/
        }
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      ...
      {view === "done" ? (
        ...
        <Landing savedStatus={savedStatus} onStart={() => setView("form")} />
      )}
    ...
    <section className="mx-auto max-w-2xl px-4 py-8">
        ...
        <div className="mb-6 flex items-start gap-3 rounded-2xl border border-primary/30 bg-primary/5 p-4">
          ...
          <div className="flex-1 text-sm">
            ...
            <p className="text-xs text-muted-foreground">
              Status: <span className="font-medium text-primary">{statusLabel(savedStatus)}</span>.
              Te contactăm pe email.
            </p>
            {savedStatus === "approved" && (
              <div className="mt-2 flex flex-wrap gap-2">
                ...
                <Link
                  ...
                  className="inline-flex items-center gap-1 rounded-full border border-primary/40 px-3 py-1.5 text-xs font-semibold text-primary"
                >
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Firme · branduri · ONG-uri · asociații · organizatori
          </p>
        </div>
      </div>
      ...
      <div className="mt-6 grid grid-cols-3 gap-3">
        <Feature
          ...
          title="Reclamă"
          desc="Discover · Events · Map"
        />
        <Feature
          icon={<Sparkles className="size-4" />}
          title="Boost evenimente"
          desc="RSVP & vizibilitate"
        />
        <Feature
          icon={<HeartHandshake className="size-4" />}
          title="Pride & ONG"
          desc="Parteneriate strategice"
        />
      </div>
      ...
      <ul className="mt-3 space-y-2 text-sm">
        ...
        <Need>
          Acceptarea{" "}
          <Link to="/legal/business-terms" className="text-primary underline">
            termenilor B2B
          </Link>{" "}
          și a{" "}
      ...
      <div className="mt-3 space-y-2">
        <Faq q="Cât durează aprobarea?">
          3 zile lucrătoare. Verificăm manual fiecare aplicant — fără excepții.
        </Faq>
        <Faq q="Pot aplica și fără cont?">
          Da, dar îți recomandăm să-ți creezi unul ca să primești update-uri și să gestionezi
          campanii.
        </Faq>
        <Faq q="Ce branduri respingeți?">
          Orice promovează terapii de conversie, dezinformare medicală, conținut anti-LGBTQ+ sau
          încalcă regulile comunității.
        </Faq>
        <Faq q="Emiteți facturi cu TVA?">
          Da, RO-eFactura pentru clienți români, reverse charge pentru UE (cu VAT valid).
        </Faq>
        <Faq q="Pot anula contractul?">
          Da, conform OUG 34/2014 — 14 zile retragere pentru servicii neîncepute. Detalii în
          Termenii B2B.
        </Faq>
        <Faq q="Datele mele sunt în siguranță?">
          Stocate în UE, criptate TLS, GDPR-compliant. Vezi{" "}
          <Link to="/legal/security-incidents" className="text-primary underline">
            procedura noastră
          </Link>
          .
        </Faq>
...
function Tier({
  ...
  return (
    <div
      className={`rounded-2xl border p-4 ${highlight ? "border-primary/50 bg-primary/5" : "border-border bg-surface/40"}`}
    >
  ...
  return (
    {
      ...
      approved: "Aprobat ✓",
      rejected: "Respins",
      needs_info: "Necesită clarificări",
    }[s] ?? s
  );
}
        ...
        <Card title="Despre organizație">
          ...
          <Field label="Denumire legală *">
            <input
              ...
              placeholder="ex: Acme Marketing SRL"
              required
            />
          </Field>
        ...
        <Card title="Obiectiv parteneriat">
          <Field label="Ce vrei să faci pe Ventuza? *">
            <textarea
              ...
              placeholder="ex: promovare bar gay-friendly București, recrutare voluntari Pride…"
              required
              maxLength={2000}
            />
          </Field>
          ...
          <Check
            ...
            label={
              <>
                ...
                și accept procesarea datelor conform GDPR (Art. 6.1.b).
              </>
            }
          />
          <Check
            ...
            label={
              <>
                Confirm că <strong>NU</strong> promovez terapii de conversie, discurs anti-LGBTQ+
...
function DoneScreen({ appId }: { appId: string | null }) {
  ...
  useEffect(() => {
    if (!appId) return;
    try {
      localStorage.setItem(STATUS_KEY, JSON.stringify({ id: appId, at: Date.now() }));
    } catch {
      /* */
    }
  }, [appId]);
      ...
      {appId && (
        <div className="w-full rounded-2xl border border-primary/30 bg-primary/5 p-4 text-left space-y-2">
          ...
          <p className="text-[11px] leading-snug text-muted-foreground">
            <strong>Păstrează acest cod.</strong> Îți va fi cerut dacă scrii la suport sau dacă vrei
            să revendici cererea dintr-un cont creat ulterior (Portal Partener → „Am deja o
            cerere").
          </p>
        </div>
      )}
      ...
      <div className="mt-2 w-full space-y-3">
        {user ? (
          <>
            ...
            <p className="text-[11px] leading-snug text-muted-foreground">
              Vei primi email când cererea e aprobată. Postările devin disponibile abia după
              aprobare.
            </p>
          </>
        ) : (
...
function Feature({ icon, title, desc }: { icon: React.ReactNode; title: string; desc: string }) {
  return (
    <div className="rounded-2xl border border-border bg-surface/60 p-3">
      <div className="flex size-8 items-center justify-center rounded-full border border-primary/30 bg-background text-primary">
        {icon}
      </div>
