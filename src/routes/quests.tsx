...
function QuestsList() {
  ...
  async function load() {
    try {
      setError(null);
      setQuests(await fetchMyQuests());
    } catch (e) {
      setError(e instanceof Error ? e.message : "Nu am putut încărca misiunile.");
    }
    setLoading(false);
  }
  ...
  async function claim(q: Quest) {
    ...
    try {
      ...
      toast.success(
        `+${q.xp_reward} XP${q.bonus_kind ? ` · ${q.bonus_amount} ${q.bonus_kind === "boost" ? "Boost" : "Super Tap"}` : ""}`,
      );
      await load();
    } catch (e) {
  ...
  return (
    <div className="space-y-2">
      <p className="px-1 pb-1 text-[11px] uppercase tracking-wider text-muted-foreground">
        Săptămâna aceasta · resetare luni
      </p>
      {quests.map((q) => {
...
function LeaderboardList() {
    ...
    return (
      <div className="px-3 py-12 text-center">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Activează locația și fii primul care intră în top.
        </p>
      </div>
    );
  ...
  return (
    <div className="space-y-1.5">
      <p className="px-1 pb-1 text-[11px] uppercase tracking-wider text-muted-foreground">
        Top 20 · rază 50 km · XP săptămâna aceasta
      </p>
      {rows.map((r) => (
        <div
          key={r.user_id}
          className={cn(
            ...
            r.rank <= 3
              ? "border-amber-500/40 bg-gradient-to-r from-amber-500/10 to-rose-500/10"
