    ...
    <div className="rounded-2xl border border-border bg-surface p-5">
      ...
      {!open && filled > 0 && (
        <div className="mt-3 flex flex-wrap gap-1.5">
          ...
          {facts.languages?.slice(0, 3).map((l) => (
            <Chip key={l} active>
              🗣 {l}
            </Chip>
          ))}
      ...
      {open && (
        <div className="mt-4 space-y-4">
          <Single
            ...
            onChange={(v) => setForm({ ...form, zodiac: v })}
          />
          <Multi
            label="Limbi"
          ...
          <Single
            ...
            onChange={(v) => setForm({ ...form, politics: v })}
          />
          <Single
            label="Copii"
            value={form.children}
          ...
          <Single
            ...
            value={form.cannabis}
            options={FREQUENCY_OPTIONS}
            onChange={(v) => setForm({ ...form, cannabis: v })}
          />
          <Single
            label="Alte droguri"
            value={form.drugs}
            options={FREQUENCY_OPTIONS}
            onChange={(v) => setForm({ ...form, drugs: v })}
          />
          <Single
            label="Sport"
            value={form.workout}
            options={WORKOUT_OPTIONS}
            onChange={(v) => setForm({ ...form, workout: v })}
          />
          <Single
            label="Dietă"
            value={form.diet}
          ...
          <Single
            ...
            onChange={(v) => setForm({ ...form, sleep_schedule: v })}
          />
...
function Multi({
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-1.5">
        {options.map((o) => (
          <Chip key={o} active={values.includes(o)} onClick={() => onChange(toggle(values, o))}>
            {o}
          </Chip>
        ))}
      </div>
