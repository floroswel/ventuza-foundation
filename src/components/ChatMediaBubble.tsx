...
export function ChatMediaBubble({ m, mine }: Props) {
  if (m.media_type === "location" && m.location_lat != null && m.location_lng != null) {
    ...
    return (
      <a href={url} target="_blank" rel="noreferrer" className={cn("flex max-w-[78%] items-center gap-3 rounded-2xl px-3 py-2.5 text-sm", mine ? "bg-primary text-primary-foreground" : "bg-muted text-foreground")}>
  ...
  return (
    <div className={cn("max-w-[78%] rounded-2xl px-3 py-2 text-sm", mine ? "bg-primary text-primary-foreground rounded-br-md" : "bg-muted text-foreground rounded-bl-md")}>
      {m.body}
    </div>
  );
}
...
function AudioBubble({ m, mine }: Props) {
  ...
  useEffect(() => { let cancelled = false; signChatMedia(m.media_url).then((u) => { if (!cancelled) setUrl(u); }); return () => { cancelled = true; }; }, [m.media_url]);
...
function ImageBubble({ m, mine }: Props) {
  ...
  useEffect(() => {
    if (!revealed) return;
    let cancelled = false;
    signChatMedia(m.media_url).then((u) => { if (!cancelled) setUrl(u); });
  ...
  useEffect(() => { if (isViewer) { setOpened(true); void markMediaViewed(m.id); } }, [isViewer, m.id]);

  if (!revealed) {
    return (
      <button onClick={() => setRevealed(true)} className={cn("flex max-w-[78%] items-center gap-3 rounded-2xl px-4 py-3 text-sm", mine ? "bg-primary/80 text-primary-foreground" : "bg-muted text-foreground")}>
  ...
  if (!mine && m.view_once && m.viewed_at && !opened) {
    return (
      <div className={cn("flex max-w-[78%] items-center gap-3 rounded-2xl px-4 py-3 text-sm", "bg-muted text-muted-foreground")}>
        <EyeOff className="size-5" /><span>Foto expirată</span>
      </div>
    );
  }

  return (
    <div className={cn("max-w-[78%] overflow-hidden rounded-2xl", mine ? "bg-primary/10" : "bg-muted")}>
      {url ? (
