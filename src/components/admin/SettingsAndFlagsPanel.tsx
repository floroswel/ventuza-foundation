  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      <NumberEditor
        label="Cooldown"
        suffix="ore"
        value={v.cooldown_hours ?? 24}
        onChange={(x) => setV({ ...v, cooldown_hours: x })}
        min={1}
        max={72}
      />
      <NumberEditor
        label="Cap zilnic / user"
        value={v.daily_cap_per_user ?? 8}
        onChange={(x) => setV({ ...v, daily_cap_per_user: x })}
        min={1}
        max={50}
      />
      <NumberEditor
        label="Rază default"
        suffix="m"
        value={v.default_radius_m ?? 2000}
        onChange={(x) => setV({ ...v, default_radius_m: x })}
        min={100}
        max={20000}
        step={100}
      />
      <NumberEditor
        label="Rază max"
        suffix="m"
        value={v.max_radius_m ?? 5000}
        onChange={(x) => setV({ ...v, max_radius_m: x })}
        min={500}
        max={50000}
        step={100}
      />
      <NumberEditor
        label="Quiet start"
        suffix="0–23"
        value={v.quiet_start_hour ?? 22}
        onChange={(x) => setV({ ...v, quiet_start_hour: x })}
        min={0}
        max={23}
      />
      <NumberEditor
        label="Quiet end"
        suffix="0–23"
        value={v.quiet_end_hour ?? 8}
        onChange={(x) => setV({ ...v, quiet_end_hour: x })}
        min={0}
        max={23}
      />
...
function QuotasForm({
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      <NumberEditor
        label="Max venues"
        value={v.max_venues ?? 10}
        onChange={(x) => setV({ ...v, max_venues: x })}
        min={1}
        max={500}
      />
      <NumberEditor
        label="Max events"
        value={v.max_events ?? 50}
        onChange={(x) => setV({ ...v, max_events: x })}
        min={1}
        max={1000}
      />
      <NumberEditor
        label="Max active offers"
        value={v.max_active_offers ?? 20}
        onChange={(x) => setV({ ...v, max_active_offers: x })}
        min={1}
        max={500}
      />
      <NumberEditor
        label="Max drafts / oră"
        value={v.max_drafts_per_hour ?? 5}
        onChange={(x) => setV({ ...v, max_drafts_per_hour: x })}
        min={1}
        max={100}
      />
      <div className="col-span-2 flex justify-end">
        ...
        <ReasonDialog
          trigger={
            <Button size="sm" variant="outline">
              <Save className="h-4 w-4 mr-1" /> Salvează JSON
            </Button>
          }
    ...
    <>
      ...
      {open && (
        <div className="mt-2 rounded-lg border border-border bg-background/40 p-2 text-xs">
          ...
          {rows && rows.length === 0 && (
            <p className="text-muted-foreground">Fără modificări înregistrate.</p>
          )}
          {rows &&
            rows.map((h) => (
              <div key={h.version} className="border-b border-border/30 py-1">
                <span className="text-muted-foreground">
                  v{h.version} · {new Date(h.changed_at ?? h.created_at).toLocaleString()}
                </span>
...
export function SettingsAndFlagsPanel() {
  ...
  return (
    <div className="space-y-6">
        ...
        {sortedSettings.length === 0 ? (
          ...
          sortedSettings.map((row) => (
            <Card key={row.key} className="p-4">
              ...
              {!["proximity_notifications", "partner_quotas", "discover.score_weights"].includes(
                row.key,
              ) &&
              ...
              {["proximity_notifications", "partner_quotas", "discover.score_weights"].includes(
                ...
                <details className="mt-3">
                  ...
                  <div className="mt-2">
                    <JsonFallbackForm row={row} onSave={(v, r) => save(row.key, v, r)} />
                  </div>
                </details>
              )}
            </Card>
      ...
      <div className="space-y-2">
        {(flags ?? []).length === 0 && (
          <p className="text-sm text-muted-foreground">Niciun feature flag (empty legitim).</p>
        )}
        {(flags ?? []).map((f) => (
          <Card key={f.key} className="p-3 flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <RolloutEditor
                flag={f}
                onSave={(pct, r) => saveFlag(f.key, { rollout_pct: pct }, r)}
              />
            </div>
          </Card>
        ))}
...
function RolloutEditor({
  ...
  flag: any;
  onSave: (pct: number, reason: string) => Promise<void>;
}) {
