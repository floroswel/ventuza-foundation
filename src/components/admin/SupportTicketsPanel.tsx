...
export function SupportTicketsPanel() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setRows(r.rows as TicketRow[]); setKpi(k);
      ...
      <GlassCard>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            ...
            <tbody>
              {rows.length === 0 && (
                <tr><td colSpan={6} className="py-6 text-center text-muted-foreground">Fără ticket-uri.</td></tr>
              )}
              {rows.map((r) => (
    ...
    <div className="fixed inset-0 z-40 flex items-end justify-center bg-background/60 backdrop-blur-md sm:items-center sm:p-6">
      <div className="max-h-[92vh] w-full max-w-2xl overflow-hidden rounded-t-2xl bg-surface shadow-xl sm:rounded-2xl">
        <div className="flex items-center justify-between border-b border-border px-4 py-3">
          ...
          <button onClick={onClose} className="rounded-md px-2 py-1 text-xs text-muted-foreground hover:bg-surface-elevated">Închide</button>
        </div>
