...
import {
  Search, Command as CmdIcon, ArrowRight, User as UserIcon, Hash,
  ShieldAlert, Ban, Zap, LayoutDashboard, CornerDownLeft,
} from "lucide-react";
...
export function CommandPaletteV2({ open, onClose, items, onSelectNav }: Props) {
  ...
  useEffect(() => {
    if (!open) { setQ(""); setEntities([]); setCursor(0); return; }
  ...
  const quickActions: Action[] = useMemo(() => {
    ...
    if (isUuid(query)) {
      acts.push({
        ...
        hint: "User 360",
        icon: UserIcon,
        perform: () => { nav({ to: "/admin/users/$id", params: { id: query } }); onClose(); },
      });
    }
    if (query.length >= 2) {
      acts.push({
        ...
        hint: "Trust & Safety",
        icon: ShieldAlert,
        perform: () => { onSelectNav("reports"); onClose(); },
      });
    }
    return acts;
  ...
  const flat: Array<{ type: "action" | "entity" | "nav"; run: () => void; key: string }> = useMemo(() => {
    ...
    entities.forEach((e) => list.push({
      type: "entity",
      key: `e-${e.id}`,
      run: () => { nav({ to: "/admin/users/$id", params: { id: e.id } }); onClose(); },
    }));
    navMatches.forEach((n) => list.push({
      type: "nav", key: `n-${n.id}`, run: () => { onSelectNav(n.id); onClose(); },
    }));
    return list;
      ...
      >
        ...
        <div className="max-h-[60vh] overflow-y-auto p-2">
          {flat.length === 0 && (
            <p className="px-4 py-8 text-center text-xs text-muted-foreground">
              {q.trim().length < 2 ? "Începe să scrii… (min 2 caractere pentru useri)" : "Niciun rezultat."}
            </p>
          )}
          ...
          {quickActions.map((a, i) => {
            ...
            return (
              <Row key={a.id} active={active} onClick={a.perform}>
                ...
                {a.hint && <span className="text-[10px] uppercase tracking-wider text-muted-foreground">{a.hint}</span>}
              </Row>
            );
          })}
          ...
          {entities.map((e, i) => {
            ...
            return (
              <Row
                key={e.id}
                active={active}
                onClick={() => { nav({ to: "/admin/users/$id", params: { id: e.id } }); onClose(); }}
              >
                <UserIcon className="size-4 text-primary" />
                <div className="flex min-w-0 flex-1 flex-col">
                  <span className="truncate text-sm">{e.title}</span>
                  <span className="truncate text-[10px] text-muted-foreground admin-mono">{e.subtitle}</span>
                </div>
                <div className="flex shrink-0 flex-wrap gap-1">
                  {e.badges.slice(0, 3).map((b) => (
                    <span key={b} className={`rounded-full px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wider ${
          ...
          {navMatches.map((n, i) => {
            ...
            const Icon = n.icon;
            return (
              <Row key={n.id} active={active} onClick={() => { onSelectNav(n.id); onClose(); }}>
        ...
        <div className="flex items-center justify-between gap-3 border-t border-border/60 bg-background/40 px-4 py-2 text-[10px] text-muted-foreground">
          <span className="flex items-center gap-3">
            <span className="flex items-center gap-1"><CornerDownLeft className="size-3" /> deschide</span>
            <span>↑↓ navighează</span>
            <span className="flex items-center gap-1"><Hash className="size-3" /> lipește UUID pentru user 360</span>
          </span>
          <span className="flex items-center gap-1"><CmdIcon className="size-3" />K</span>
        </div>
      </div>
    </div>
  );
