...
import {
  ...
  adminEnqueueNcmec,
  adminMarkNcmecFiled,
  adminGenerateArt33Draft,
  adminUpdateBreach,
  adminAddBreachEvent,
  adminListDsaSor,
  adminCreateDsaSor,
  adminExportDsaTransparency,
  adminListCountryRisk,
  adminUpsertCountryRisk,
  adminDeleteCountryRisk,
  adminListEfacturaQueue,
  adminGenerateEfacturaXml,
  adminMarkEfacturaSubmitted,
  ...
  return (
    <div className="rounded-lg border border-red-500/40 bg-red-500/5 p-3 text-sm text-red-300">
      <p className="font-medium">
        {/forbidden|denied|role/i.test(error) ? "Acces refuzat" : "Eroare"}
      </p>
...
function NcmecTab() {
  ...
  const load = async () => {
    ...
    try {
      const [q, r] = await Promise.all([
        ...
        supabase
          .from("csam_reports")
          .select("id, status, notes, reported_at, hash")
          .eq("status", "pending")
  ...
  const submit = async () => {
    if (!selected || narrative.length < 20) {
      toast.error("Selectează raport + narativ ≥ 20 caractere");
      return;
    }
    try {
      await enqueue({
        data: { csamReportId: selected, narrative, country: country.toUpperCase() },
      });
    ...
    <div className="space-y-4">
      <GlassCard>
        ...
        <div className="grid gap-3 md:grid-cols-2">
            ...
            >
              ...
              {reports.map((r) => (
                <option key={r.id} value={r.id}>
                  {new Date(r.reported_at).toLocaleDateString("ro-RO")} · hash{" "}
                  {String(r.hash ?? "").slice(0, 12)}…
                </option>
              ))}
            </select>
          </div>
          <div>
            <label className="text-xs text-muted-foreground">
              Țara raportorului / conținutului (ISO2)
            </label>
            <input
              value={country}
        ...
        <label className="mt-3 block text-xs text-muted-foreground">
          Narativ CyberTipline (≥ 20 caractere)
        </label>
        <textarea
          value={narrative}
        ...
        <p className="mt-2 text-xs text-muted-foreground">
          Submisia finală la NCMEC CyberTipline se face manual (encrypted email / API key). Aici
          pregătim pachetul + urmă audit.
        </p>
      </GlassCard>
          ...
          {rows.map((r) => (
            <div key={r.id} className="rounded-lg border border-border/40 p-3 text-xs">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {r.affected_country && (
                  <span className="text-muted-foreground">· {r.affected_country}</span>
                )}
                {r.ncmec_case_id && (
                  <span className="ml-auto font-mono">Case: {r.ncmec_case_id}</span>
                )}
              </div>
              ...
              {r.status === "queued" && (
                <div className="mt-2 flex gap-2">
                  <input
                    placeholder="NCMEC Case ID"
                    value={caseId[r.id] ?? ""}
                    onChange={(e) => setCaseId({ ...caseId, [r.id]: e.target.value })}
                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        await markFiled({ data: { queueId: r.id, ncmecCaseId: caseId[r.id] } });
                        toast.success("Marcat filed");
                        load();
                      } catch (e: any) {
                        toast.error(e.message);
                      }
...
function BreachTab() {
  ...
  useEffect(() => {
    if (sel) {
      const fresh = rows.find((r) => r.id === sel.id);
  ...
  const createNew = async () => {
    ...
    if (e) toast.error(e.message);
      ...
      toast.success("Incident creat — start ceas 72h");
      load();
    }
  };
    ...
    <div className="grid gap-4 md:grid-cols-[280px_1fr]">
      <GlassCard>
        <div className="mb-3 flex items-center justify-between">
          ...
          <button onClick={createNew} className="rounded bg-red-600 px-2 py-1 text-xs text-white">
            + Nou
          </button>
        </div>
          ...
          {rows.map((r) => {
            ...
            return (
              ...
              >
                ...
                <p
                  className={`mt-0.5 ${overdue ? "text-red-400" : hoursLeft < 24 ? "text-amber-400" : "text-muted-foreground"}`}
                >
                  {r.authority_notified_at
                    ? "✓ notificat"
                    : overdue
                      ? `⚠️ DEPĂȘIT ${Math.abs(hoursLeft)}h`
                      : `${hoursLeft}h până termen`}
                </p>
      ...
      {sel ? (
        <GlassCard>
          <BreachEditor
            ...
            onSave={async (patch) => {
              try {
                await update({ data: { breachId: sel.id, patch } });
            ...
            onGenerate={async () => {
              try {
                const r = await generate({ data: { breachId: sel.id } });
                toast.success("Draft-uri generate");
                setSel({ ...sel, art33_draft: r.art33_draft, art34_draft: r.art34_draft });
                load();
              } catch (e: any) {
                toast.error(e.message);
              }
            ...
            onAddEvent={async (kind, note) => {
              try {
                await addEvent({ data: { breachId: sel.id, kind, note } });
                toast.success("Adăugat în timeline");
                load();
              } catch (e: any) {
                toast.error(e.message);
              }
        ...
        <GlassCard>
          <p className="text-sm text-muted-foreground">
            Selectează un incident sau creează unul nou.
          </p>
        </GlassCard>
      )}
    </div>
...
function BreachEditor({
  ...
  onGenerate: () => void;
  onAddEvent: (kind: string, note: string) => void;
}) {
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-xs text-muted-foreground">
          Descoperit: {new Date(incident.discovered_at).toLocaleString("ro-RO")} · Termen ANSPDCP:{" "}
          {dueLabel}
        </p>
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <label className="text-xs">
          ...
          <input
            ...
            className="mt-1 w-full rounded border border-border/40 bg-background p-2 text-sm"
          />
        </label>
        <label className="text-xs">
          Severitate
        ...
        <label className="text-xs">
          Useri afectați (aprox)
          <input
            type="number"
            value={form.affected_users_count ?? ""}
            onChange={(e) => set("affected_users_count", parseInt(e.target.value) || null)}
            className="mt-1 w-full rounded border border-border/40 bg-background p-2 text-sm"
          />
        </label>
        <label className="text-xs md:col-span-2">
          Categorii date (virgulă)
          <input
      ...
      <div className="flex flex-wrap gap-2">
        <button
          onClick={() =>
            onSave({
              ...
              containment_actions: form.containment_actions,
        ...
        <button
          onClick={onGenerate}
          className="rounded border border-border/40 px-3 py-1.5 text-sm"
        >
          Generează draft Art. 33/34
        </button>
        {!form.authority_notified_at && (
          <button
            onClick={() =>
              onSave({ authority_notified_at: new Date().toISOString(), status: "notified" })
            }
            className="rounded bg-emerald-600 px-3 py-1.5 text-sm text-white"
          >
            Marchează ANSPDCP notificat
          </button>
        )}
        {!form.closed_at && (
          <button
            onClick={() => onSave({ closed_at: new Date().toISOString(), status: "closed" })}
            className="rounded border border-border/40 px-3 py-1.5 text-sm"
          >
            Închide incident
          </button>
      ...
      {form.art33_draft && (
        <details className="rounded-lg border border-border/40 p-3">
          ...
          <textarea
            value={form.art33_draft}
            onChange={(e) => set("art33_draft", e.target.value)}
            rows={16}
            className="mt-2 w-full rounded border border-border/40 bg-background p-2 font-mono text-xs"
          />
        </details>
      )}
      {form.art34_draft && (
        <details className="rounded-lg border border-border/40 p-3">
          <summary className="cursor-pointer text-sm font-medium">
            Draft Art. 34 (userii vizați)
          </summary>
          <textarea
            value={form.art34_draft}
            onChange={(e) => set("art34_draft", e.target.value)}
            rows={12}
            className="mt-2 w-full rounded border border-border/40 bg-background p-2 text-xs"
          />
        </details>
      )}
      ...
      <div className="rounded-lg border border-border/40 p-3">
        ...
        <div className="mt-2 flex gap-2">
          <select
            ...
            className="rounded border border-border/40 bg-background p-1.5 text-xs"
          >
...
function DsaSorTab() {
  ...
  const submit = async () => {
    if (!form.category || form.reasoning.length < 20) {
      toast.error("Categoria + reasoning ≥ 20 caractere");
      return;
    }
    try {
      await create({
      ...
      toast.success("SoR creat");
      setForm({ ...form, category: "", reasoning: "" });
      load();
    } catch (e: any) {
      toast.error(e.message);
    }
    ...
    <div className="space-y-4">
      <GlassCard>
        ...
        <div className="grid gap-3 md:grid-cols-3">
          ...
          <label className="text-xs">
            Categorie (ex: hate_speech, csam, spam)
            <input
              value={form.category}
          ...
          <label className="text-xs">
            Content type
            <input
              value={form.target_content_type}
              onChange={(e) => setForm({ ...form, target_content_type: e.target.value })}
              placeholder="messages | profile | photo"
              className="mt-1 w-full rounded border border-border/40 bg-background p-2 text-sm"
            />
          </label>
          <label className="text-xs">
            Content ID
            <input
              value={form.target_content_id}
              onChange={(e) => setForm({ ...form, target_content_id: e.target.value })}
              className="mt-1 w-full rounded border border-border/40 bg-background p-2 text-sm"
            />
          </label>
        </div>
        <label className="mt-3 block text-xs">
          Reasoning (motivare — DSA cere claritate)
          <textarea
        ...
        <label className="mt-2 flex items-center gap-2 text-xs">
          <input
            ...
            onChange={(e) => setForm({ ...form, automated: e.target.checked })}
          />{" "}
          Decizie automată
        </label>
        <button
      ...
      <GlassCard>
        ...
        <div className="flex flex-wrap items-end gap-2">
          <label className="text-xs">
            ...
            <input
              ...
              className="ml-2 w-24 rounded border border-border/40 bg-background p-1.5 text-sm"
            />
          </label>
          <label className="text-xs">
            Lună (opt){" "}
            <select
              ...
              className="ml-2 rounded border border-border/40 bg-background p-1.5 text-sm"
            >
        ...
        {report && (
          <div className="mt-3 text-xs">
            <p>
              Total acțiuni: <span className="font-mono">{report.total}</span>
            </p>
            <div className="mt-2 space-y-1">
              {Object.entries(report.breakdown as Record<string, number>).map(([k, v]) => (
                <div key={k} className="flex justify-between border-b border-border/20 py-1">
        ...
        <div className="max-h-96 space-y-1.5 overflow-auto text-xs">
          {rows.map((r) => (
            <div key={r.id} className="rounded border border-border/40 p-2">
              <div className="flex flex-wrap items-center gap-2">
                <span className="rounded bg-primary/10 px-1.5 font-medium text-primary">
                  {r.action_type}
                </span>
                <span className="rounded bg-muted px-1.5">{r.legal_basis}</span>
                <span className="rounded bg-muted px-1.5">{r.category}</span>
                {r.automated && (
                  <span className="rounded bg-amber-500/20 px-1.5 text-amber-300">AUTO</span>
                )}
          ...
          <table className="w-full text-xs">
            <thead className="text-muted-foreground">
              <tr className="border-b border-border/40">
                ...
                <th className="p-1.5 text-left">Țară</th>
                <th className="p-1.5 text-left">Risc</th>
                <th className="p-1.5 text-center">Hide loc</th>
                <th className="p-1.5 text-center">Stealth</th>
                <th className="p-1.5 text-center">No discover</th>
                <th className="p-1.5 text-center">No signup</th>
                <th></th>
              </tr>
            </thead>
            <tbody>
                  ...
                  <td className="p-1.5">
                    ...
                    <button
                      onClick={async () => {
                        ...
                        try {
                          await del({ data: { country_code: r.country_code } });
                          toast.success("Șters");
                          load();
                        } catch (e: any) {
                          toast.error(e.message);
      ...
      {edit && (
        <GlassCard>
          <p className="mb-3 text-sm font-medium">
            {edit.country_code ? `Editează ${edit.country_code}` : "Adaugă țară"}
          </p>
          <div className="grid gap-3 md:grid-cols-3">
            <label className="text-xs">
              Cod ISO-2
              <input
                ...
                onChange={(e) => setEdit({ ...edit, country_code: e.target.value.toUpperCase() })}
                maxLength={2}
                className="mt-1 w-full rounded border border-border/40 bg-background p-2 uppercase"
              />
            </label>
            <label className="text-xs">
              Nume țară
              <input
                ...
                onChange={(e) => setEdit({ ...edit, country_name: e.target.value })}
                className="mt-1 w-full rounded border border-border/40 bg-background p-2"
              />
            </label>
            <label className="text-xs">
              Nivel risc
              <select
                ...
                onChange={(e) => setEdit({ ...edit, risk_level: e.target.value })}
                className="mt-1 w-full rounded border border-border/40 bg-background p-2"
              >
            ...
            <label className="flex items-center gap-2 text-xs">
              <input
                type="checkbox"
                checked={edit.hide_precise_location}
                onChange={(e) => setEdit({ ...edit, hide_precise_location: e.target.checked })}
              />{" "}
              Ascunde locație precisă
            </label>
            <label className="flex items-center gap-2 text-xs">
              <input
                type="checkbox"
                checked={edit.force_stealth}
                onChange={(e) => setEdit({ ...edit, force_stealth: e.target.checked })}
              />{" "}
              Forțează stealth
            </label>
            <label className="flex items-center gap-2 text-xs">
              <input
                type="checkbox"
                checked={edit.disable_discover}
                onChange={(e) => setEdit({ ...edit, disable_discover: e.target.checked })}
              />{" "}
              Dezactivează Discover
            </label>
            <label className="flex items-center gap-2 text-xs">
              <input
                type="checkbox"
                checked={edit.disable_signup}
                onChange={(e) => setEdit({ ...edit, disable_signup: e.target.checked })}
              />{" "}
              Blochează signup
            </label>
    ...
    <div className="space-y-3">
      <GlassCard>
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Generăm UBL 2.1 (CIUS-RO). Upload la SPV se face manual sau prin OAuth ANAF (necesită
          certificat digital calificat + acces SPV).
        </p>
      </GlassCard>
      <div className="space-y-2">
        {rows.map((r) => {
          ...
          return (
            <GlassCard key={r.id}>
              <div className="flex flex-wrap items-center gap-2 text-xs">
                ...
                <span
                  className={`ml-auto rounded px-1.5 font-medium ${
                    r.status === "accepted"
                      ? "bg-emerald-500/20 text-emerald-300"
                      : r.status === "rejected"
                        ? "bg-red-500/20 text-red-300"
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                <button
                  onClick={async () => {
                    try {
                      const res = await gen({ data: { queueId: r.id } });
                      setXml({ ...xml, [r.id]: res.xml });
                      toast.success("XML generat");
                      load();
                    } catch (e: any) {
                      toast.error(e.message);
                    }
                ...
                {xml[r.id] && (
                  <button
                    onClick={() => downloadXml(label, xml[r.id])}
                    className="inline-flex items-center gap-1 rounded bg-primary px-2 py-1 text-xs text-primary-foreground"
                  >
                ...
                <input
                  placeholder="ANAF upload ID"
                  value={uploadId[r.id] ?? ""}
                  onChange={(e) => setUploadId({ ...uploadId, [r.id]: e.target.value })}
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      await mark({
                        data: { queueId: r.id, anafUploadId: uploadId[r.id], status: "submitted" },
                      });
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      await mark({
                        data: { queueId: r.id, anafUploadId: uploadId[r.id], status: "accepted" },
                      });
              ...
              {xml[r.id] && (
                <details className="mt-2">
                  ...
                  <pre className="mt-1 max-h-64 overflow-auto rounded bg-background/50 p-2 font-mono text-[10px]">
                    {xml[r.id]}
                  </pre>
                </details>
              )}
        ...
        {rows.length === 0 && (
          <p className="text-xs text-muted-foreground">Nicio factură în coadă.</p>
        )}
      </div>
    </div>
  );
