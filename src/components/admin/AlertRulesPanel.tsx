...
export function AlertRulesPanel() {
  ...
  const load = async () => {
    setError(null);
    try { setRules((await list()) as Rule[]); }
    catch (e: any) { setError(e?.message ?? "Nu am putut încărca regulile"); }
  ...
  const doSimulate = async () => {
    if (!editing?.event_kind) { toast.error("Alege event_kind"); return; }
    try {
    ...
    } catch (e: any) { toast.error(e?.message ?? "Eroare simulare"); }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={() => { setEditing({ ...EMPTY_RULE }); setSim(null); }}
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed border-border p-6 text-center text-sm text-muted-foreground">
          Nicio regulă activă. Creează una pentru a primi alerte automate când un tip de eveniment depășește pragul.
        </div>
      ) : (
            ...
            <tbody>
              {rules.map((r) => (
                <tr key={r.id} className="border-t border-border">
                  ...
                  <td className="px-3 py-2 text-xs">≥ {r.threshold} / {humanWindow(r.window_seconds)}</td>
                  <td className="px-3 py-2">
                    <span className={`rounded-full border px-2 py-0.5 text-xs ${SEV_COLOR[r.severity]}`}>{r.severity}</span>
                  </td>
                  ...
                  <td className="px-3 py-2 text-right">
                    <button
                      onClick={() => { setEditing(r); setSim(null); }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-2xl rounded-2xl border border-border bg-background p-5 shadow-xl">
            <div className="mb-3 flex items-center justify-between">
              <h3 className="text-base font-semibold">{editing.id ? "Editează regula" : "Regulă nouă"}</h3>
              <button onClick={() => { setEditing(null); setSim(null); }} className="rounded-full p-1 hover:bg-surface"><X className="size-4" /></button>
            </div>
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <label className="text-xs">
                <span className="mb-1 block text-muted-foreground">Nume *</span>
                <input value={editing.name ?? ""} onChange={(e) => setEditing({ ...editing, name: e.target.value })}
                  className="w-full rounded-md border border-border bg-background px-3 py-2" placeholder="Ex: Rapoarte multiple în 1h" />
              </label>
              <label className="text-xs">
                <span className="mb-1 block text-muted-foreground">Event kind *</span>
                <input value={editing.event_kind ?? ""} onChange={(e) => setEditing({ ...editing, event_kind: e.target.value })}
                  list="event-kinds" className="w-full rounded-md border border-border bg-background px-3 py-2 font-mono" placeholder="fingerprint_cluster" />
                <datalist id="event-kinds">
                  {eventKinds.map((k) => <option key={k} value={k} />)}
                </datalist>
              </label>
              ...
              <label className="text-xs">
                ...
                <input type="number" min={1} value={editing.threshold ?? 1}
              ...
              <label className="text-xs">
                ...
                <input type="number" min={60} value={editing.window_seconds ?? 3600}
                  onChange={(e) => setEditing({ ...editing, window_seconds: Number(e.target.value) })}
                  className="w-full rounded-md border border-border bg-background px-3 py-2" />
              </label>
              <label className="text-xs">
                <span className="mb-1 block text-muted-foreground">Severitate</span>
                <select value={editing.severity ?? "warn"} onChange={(e) => setEditing({ ...editing, severity: e.target.value as Rule["severity"] })}
                  className="w-full rounded-md border border-border bg-background px-3 py-2">
              ...
              <label className="text-xs">
                ...
                <select value={String(editing.enabled ?? true)} onChange={(e) => setEditing({ ...editing, enabled: e.target.value === "true" })}
                  className="w-full rounded-md border border-border bg-background px-3 py-2">
            ...
            {sim && (
              <div className={`mt-3 rounded-lg border p-3 text-xs ${sim.would_fire ? "border-orange-500/40 bg-orange-500/10 text-orange-500" : "border-border bg-surface text-muted-foreground"}`}>
            ...
            <div className="mt-4 flex items-center justify-end gap-2">
              <button onClick={doSimulate} className="rounded-md border border-border px-3 py-1.5 text-xs hover:bg-surface">
                <PlayCircle className="mr-1 inline size-3" /> Simulează
              </button>
              <button onClick={() => { setEditing(null); setSim(null); }} className="rounded-md border border-border px-3 py-1.5 text-xs hover:bg-surface">Anulează</button>
              <button onClick={save} disabled={saving} className="rounded-md bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground disabled:opacity-50">
