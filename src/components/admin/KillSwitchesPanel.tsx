...
const KEYS: {
  key: "chat" | "matching" | "uploads" | "discover" | "signup" | "partner_portal";
  label: string;
}[] = [
  { key: "chat", label: "Chat" },
...
export function KillSwitchesPanel() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setMv(v);
      const vAny = v as any;
      setWeb(vAny?.web ?? "0.0.0");
      setIos(vAny?.ios ?? "0.0.0");
      setAndroid(vAny?.android ?? "0.0.0");
    ...
    <div className="space-y-4">
      ...
      <GlassCard>
        <p className="mb-3 text-xs text-muted-foreground">
          Dezactivează instant module la nivel de app. Se logă în <code>admin_audit_log</code> ca{" "}
          <b>critical</b>.
        </p>
        <div className="grid gap-2 sm:grid-cols-2">
          {KEYS.map(({ key, label }) => {
            ...
            return (
              <div
                key={key}
                className="flex items-center justify-between rounded-xl border border-border/60 bg-surface-elevated p-3"
              >
                <div>
                  <p className="text-sm font-medium">{label}</p>
                  <StatusBadge tone={on ? "rejected" : "approved"}>
                    {on ? "DEZACTIVAT" : "activ"}
                  </StatusBadge>
                </div>
                <ReasonDialog
                  ...
                  description={
                    on
                      ? "Modulul redevine disponibil userilor."
                      : "TOȚI userii vor pierde accesul la acest modul instant."
                  }
      ...
      <GlassCard>
        <div className="grid gap-2 sm:grid-cols-3">
          ...
          <div>
            <Label className="text-xs">iOS</Label>
            <Input
              value={ios}
          ...
          <div>
            <Label className="text-xs">Android</Label>
            <Input
              value={android}
        ...
        <div className="mt-3 flex items-center justify-between">
          <p className="text-[11px] text-muted-foreground">
            Actualmente: web {(mv as any)?.web} · ios {(mv as any)?.ios} · android{" "}
            {(mv as any)?.android}
          </p>
          <ReasonDialog
            ...
            onConfirm={async (reason) => {
              const r = await setMV({
                data: { web, ios, android, force_update_message: msg, reason },
              });
              setMv(r.value);
            }}
          />
