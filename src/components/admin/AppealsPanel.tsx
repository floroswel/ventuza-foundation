...
export function AppealsPanel() {
  ...
  const load = async () => {
    setBusy(true);
    try {
      const r = await list({ data: { status: status as any, limit: 100 } });
      setRows(r.rows as Appeal[]);
    } catch (e: any) {
    ...
    <div className="space-y-4">
      ...
      <GlassCard>
        <div className="mb-3 flex flex-wrap items-center gap-2">
          ...
          >
            {["open", "under_review", "granted", "denied", "withdrawn", "all"].map((s) => (
        ...
        {rows.length === 0 && (
          <p className="py-6 text-center text-xs text-muted-foreground">Nicio contestație.</p>
        )}
        <ul className="space-y-2">
          {rows.map((a) => (
            <li
              key={a.id}
              className="rounded-xl border border-border/60 bg-surface-elevated p-3 text-sm"
            >
              ...
              {a.original_reason && (
                <p className="mb-1 text-[11px] text-muted-foreground">
                  <span className="uppercase tracking-wider">Motiv inițial:</span>{" "}
                  {a.original_reason}
                </p>
              )}
              <p className="whitespace-pre-wrap rounded-md bg-background/50 p-2 text-xs">
                {a.user_statement}
              </p>
