...
import { getRevenueStats, getRetentionCohorts, getFunnelStats } from "@/lib/admin-intelligence.functions";
...
export function IntelligenceDashboardPanel() {
  ...
  const load = async () => {
    ...
    try {
      const [r, c, f] = await Promise.all([rev(), coh({ data: { days: 30 } }), fun({ data: { days: 30 } })]);
      setRevenue(r); setCohorts(c as any[]); setFunnel(f);
  ...
  return (
    <div className="space-y-4">
      <SectionTitle action={
        <button onClick={load} className="inline-flex items-center gap-1 rounded-full border border-border px-2.5 py-1 text-[10px] hover:border-primary/50">
          <RefreshCw className="size-3" /> Refresh
        </button>
      }><TrendingUp className="mr-1 inline size-4" />Business intelligence</SectionTitle>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead className="text-[10px] uppercase text-muted-foreground">
                <tr><th className="px-2 py-1 text-left">Cohortă</th><th>N</th><th>D1</th><th>D7</th><th>D30</th></tr>
              </thead>
              <tbody>
                {cohorts.length === 0 && <tr><td colSpan={5} className="py-4 text-center text-muted-foreground">Fără date.</td></tr>}
                {cohorts.map((c: any) => (
                  <tr key={c.cohort_date} className="border-t border-border/50">
                    <td className="px-2 py-1">{new Date(c.cohort_date).toLocaleDateString("ro-RO")}</td>
                    <td className="px-2 py-1 text-center"><MonoNumber value={c.cohort_size} /></td>
                ...
                return (
                  <li key={k as string}>
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="admin-mono">{Number(v).toLocaleString("ro-RO")} · {pct}%</span>
                    </div>
