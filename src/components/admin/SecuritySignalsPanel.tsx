...
export function SecuritySignalsPanel() {
  ...
  return (
    <div className="space-y-4">
      ...
      <PanelStatus state={state} retry={reload} isEmpty={false}>
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="space-y-2">
          {data.alerts.map((a, i) => (
            ...
            >
              ...
              <div>
                <div className="font-medium uppercase tracking-wide text-[10px] opacity-70">
                  {a.kind}
                </div>
                <div>{a.message}</div>
              </div>
      ...
      <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
        <Kpi
          label="Signup ultima oră"
          value={s.signups_last_hour}
          hint={`vs ${s.signups_prev_hour} h-1 (${spikePct > 0 ? "+" : ""}${spikePct}%)`}
          icon={<TrendingUp className="size-3" />}
          tone={spikePct > 200 ? "danger" : "default"}
        />
        <Kpi
          label={`Signup ${data.window_hours}h`}
          value={s.signups_total}
          hint={`medie ${s.signups_avg_per_hour}/h`}
        ...
        <Kpi
          ...
          tone={s.high_risk_count >= 10 ? "danger" : s.high_risk_count > 0 ? "warn" : "default"}
        />
        <Kpi
          label="Fingerprints duplicate"
          value={s.duplicate_fingerprint_count}
          icon={<Fingerprint className="size-3" />}
          tone={s.duplicate_fingerprint_count >= 3 ? "warn" : "default"}
        />
      </div>
          ...
          <table className="w-full text-xs">
            <thead className="text-muted-foreground">
              <tr>
                ...
                <th className="text-right p-1">Risk</th>
            ...
            <tbody>
              {data.high_risk_users.map((u) => (
                <tr key={u.user_id} className="border-t border-border/40">
                  <td className="p-1">
                    <div>{u.display_name ?? "(fără nume)"}</div>
                  ...
                  <td className="p-1 text-right">
                    <span
                      className={`rounded px-2 py-0.5 ${u.risk_score >= 80 ? "bg-red-500/20 text-red-300" : "bg-amber-500/20 text-amber-300"}`}
                    >
                      {u.risk_score}
                    </span>
                  </td>
      ...
      <Section
        title="Fingerprints folosite de mai multe conturi"
      ...
      >
        {data.duplicate_fingerprints.length === 0 ? (
          ...
          <table className="w-full text-xs">
            <thead className="text-muted-foreground">
              <tr>
                <th className="text-left p-1">Fingerprint</th>
                <th className="text-right p-1">Conturi</th>
                <th className="text-left p-1">Ultima utilizare</th>
              </tr>
            </thead>
            <tbody>
            ...
            <table className="w-full text-xs">
              <thead className="sticky top-0 bg-surface text-muted-foreground">
                <tr>
                  <th className="text-left p-1">User</th>
                  <th className="text-right p-1">Risk</th>
                  <th className="text-left p-1">Vârstă</th>
                  <th className="text-left p-1">Creat</th>
                </tr>
              </thead>
              <tbody>
