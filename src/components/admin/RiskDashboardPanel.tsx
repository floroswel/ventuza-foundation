...
export function RiskDashboardPanel() {
  ...
  const [state, reload, lastLoadedAt] = useAdminPanelLoad<Dashboard>(
    async () => {
      const { data, error } = await (supabase as any).rpc("admin_risk_dashboard", {
  ...
  useEffect(() => {
    const ch = supabase
      ...
      .on(
        "postgres_changes",
        { event: "UPDATE", schema: "public", table: "profiles", filter: "risk_score=gt.59" },
        () => {
          reload();
        },
      )
...
function Content({ data, onOpenUser }: { data: Dashboard; onOpenUser: (id: string) => void }) {
  ...
  const maxDist = useMemo(
    () => Math.max(...data.distribution.map((b) => b.count), 1),
    [data.distribution],
  );
  const maxTrendH = useMemo(
    () => Math.max(...data.trend_hourly.map((b) => b.flags), 1),
    [data.trend_hourly],
  );
  const maxTrendD = useMemo(
    () => Math.max(...data.trend_daily.map((b) => b.flags), 1),
    [data.trend_daily],
  );
  ...
  useEffect(() => {
    setPage(0);
  }, [search, scoreBucket, userStatus, fingerprintUserIds]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
        ...
        <Kpi
          label={`Flag-uri (${data.window_hours}h)`}
          value={s.flags_window}
          icon={<Flag className="size-3" />}
        />
        <Kpi
          label="Flag-uri deschise"
          value={s.flags_open}
          icon={<Flag className="size-3" />}
          tone={s.flags_open > 10 ? "warn" : "default"}
        />
        <Kpi
          label="Flag-uri ultima oră"
          value={s.flags_last_hour}
          icon={<TrendingUp className="size-3" />}
          tone={s.flags_last_hour >= 5 ? "danger" : s.flags_last_hour > 0 ? "warn" : "default"}
        />
        <Kpi
          label="Generat"
          value={fmt(data.generated_at)}
      ...
      <div className="rounded-lg border border-border bg-surface p-3">
        ...
        <div className="grid gap-2 md:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="flex flex-wrap gap-2 text-[10px]">
            <FilterChip label="Scor">
              {(["all", "0-39", "40-59", "60-79", "80+"] as ScoreBucket[]).map((b) => (
                <ChipButton key={b} active={scoreBucket === b} onClick={() => setScoreBucket(b)}>
                  {b === "all" ? "toate" : b}
                </ChipButton>
              ))}
            </FilterChip>
          ...
          <div className="flex flex-wrap gap-2 text-[10px] lg:col-span-3">
            <FilterChip label="Tip semnal">
              <ChipButton active={flagKind === "all"} onClick={() => setFlagKind("all")}>
                toate
              </ChipButton>
              {data.kinds.map((k) => (
                <ChipButton
                  key={k.kind}
                  active={flagKind === k.kind}
                  onClick={() => setFlagKind(k.kind)}
                >
        ...
        {fingerprintError && (
          <div className="mt-2 text-[10px] text-red-300">
            Eroare fingerprint: {fingerprintError}
          </div>
        )}
        {fingerprintUserIds && (
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <Section title="Tendință 24h (per oră)" icon={<TrendingUp className="size-4" />}>
          {data.trend_hourly.length === 0 ? (
            <Empty label="Niciun flag în 24h." />
          ) : (
        ...
        <Section title="Tendință 30z (per zi)" icon={<TrendingUp className="size-4" />}>
          {data.trend_daily.length === 0 ? (
            <Empty label="Niciun flag în 30z." />
          ) : (
            <div className="flex items-end gap-1 overflow-x-auto py-2">
      ...
      {/* Kinds breakdown */}
      <Section
        title={`Tipuri de semnale (${data.window_hours}h)`}
        icon={<Flag className="size-4" />}
      >
        {data.kinds.length === 0 ? (
          <Empty label="Niciun semnal în fereastră." />
        ) : (
            ...
            {data.kinds.map((k) => (
              ...
              >
                ...
                <div className="text-muted-foreground">
                  {k.count} · sev. medie {k.avg_severity}
                </div>
              </div>
            ))}
          </div>
        ...
        {filteredUsers.length === 0 ? (
          ...
          <>
                  ...
                  {pagedUsers.map((u) => (
                    ...
                    >
                      ...
                      <td className="p-1 text-right">
                        <span className={`rounded border px-2 py-0.5 ${scoreTone(u.risk_score)}`}>
                          {u.risk_score}
                        </span>
                      </td>
                      ...
                      <td className="p-1">
                        {u.banned_at ? (
                          <span className="text-red-400">banat</span>
                        ) : u.suspended_until ? (
            ...
            <div className="mt-2 flex items-center justify-between text-[10px] text-muted-foreground">
              <span>
                Pagina {pageSafe + 1} / {totalPages} · {pageSafe * PAGE_SIZE + 1}–
                {Math.min((pageSafe + 1) * PAGE_SIZE, filteredUsers.length)} din{" "}
      ...
      >
        {filteredFlags.length === 0 ? (
          <Empty
            label={
              filtersActive
                ? "Niciun semnal nu corespunde filtrelor."
                : "Niciun semnal în fereastră."
            }
          />
        ) : (
          <div className="max-h-96 overflow-y-auto">
              ...
              <tbody>
                {filteredFlags.map((f) => (
                  ...
                  >
                    ...
                    <td className="p-1">
                      <span className="rounded bg-primary/10 px-2 py-0.5 uppercase text-[10px]">
                        {f.kind}
                      </span>
                    </td>
                    <td className="p-1 text-right">
                      <span className={`rounded border px-2 py-0.5 ${scoreTone(f.severity)}`}>
                        {f.severity}
                      </span>
                    </td>
                    <td className="p-1">
