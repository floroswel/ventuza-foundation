...
import { GlassCard, MonoNumber, Kpi, StatusBadge, SectionTitle } from "@/components/admin/ui/primitives";

const RON = (minor: number | null | undefined) =>
  ((minor ?? 0) / 100).toLocaleString("ro-RO", { style: "currency", currency: "RON" });

type Invoice = {
  id: string; owner_id: string; series: string; number: number; year: number;
  payment_code: string; kind: string; plan_code: string | null;
  total_minor: number; subtotal_minor: number; vat_minor: number;
  status: string; issued_at: string; due_at: string; paid_at: string | null; paid_ref: string | null;
  billing_snapshot: any; issuer_snapshot: any;
};
...
export function InvoicesPanel() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const r = await list({ data: { status: filterStatus || undefined, search: search || undefined, limit: 200 } });
  ...
  const onCancel = async (inv: Invoice) => {
    ...
    try { await cancel({ data: { invoice_id: inv.id, reason } }); toast.success("Factură anulată"); load(); }
  ...
  if (error) return (
    <div className="p-4 rounded bg-destructive/10 text-destructive">
      ...
      <Button size="sm" variant="outline" onClick={load} className="mt-2">Reîncearcă</Button>
    </div>
  );
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2 items-end">
        ...
        <div className="flex-1 min-w-[200px]">
          ...
          <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="VTZ-2026-…" />
        </div>
        <Button variant="outline" onClick={load}><RefreshCw className="h-4 w-4 mr-1" />Refresh</Button>
      </div>

      {rows.length === 0 ? (
        <GlassCard><div className="p-6 text-sm text-muted-foreground">Nicio factură (empty legitim).</div></GlassCard>
      ) : (
        <div className="rounded border overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-muted/40">
              <tr className="text-left">
                <th className="p-2">Cod plată</th><th className="p-2">Tip</th><th className="p-2">Plan</th>
                <th className="p-2">Partener (CUI)</th><th className="p-2 text-right">Total</th>
                <th className="p-2">Status</th><th className="p-2">Emisă</th><th className="p-2">Scadență</th>
                ...
                <tr key={inv.id} className="border-t hover:bg-muted/30">
                  ...
                  <td className="p-2 flex gap-1">
                    {(inv.status === "pending_payment" || inv.status === "overdue") && (
                      <>
                        <Button size="sm" onClick={() => setSelected(inv)}><CheckCircle2 className="h-3 w-3 mr-1" />Confirmă</Button>
                        <Button size="sm" variant="outline" onClick={() => onCancel(inv)}><XCircle className="h-3 w-3" /></Button>
                      </>
                    )}
                    <Button size="sm" variant="ghost" onClick={() => setSelected(inv)}><FileText className="h-3 w-3" /></Button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {selected && <InvoiceDrawer invoice={selected} onClose={() => setSelected(null)} onChanged={load} />}
    </div>
  );
}

function InvoiceDrawer({ invoice, onClose, onChanged }: { invoice: Invoice; onClose: () => void; onChanged: () => void }) {
  ...
  const onConfirm = async () => {
    ...
    try {
      ...
      await confirm({ data: { invoice_id: invoice.id, amount_minor: minor, paid_ref: paidRef, paid_at: new Date(paidAt).toISOString() } });
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>Tip: {invoice.kind} · Plan: {invoice.plan_code ?? "—"}</DialogDescription>
        </DialogHeader>
        <div className="space-y-3 text-sm">
          <div className="grid grid-cols-2 gap-3">
            <GlassCard><div className="p-3">
              <div className="text-xs text-muted-foreground mb-1">EMITENT</div>
              <div className="font-medium">{invoice.issuer_snapshot?.name}</div>
              <div className="text-xs">CUI {invoice.issuer_snapshot?.cui} · {invoice.issuer_snapshot?.reg_com}</div>
            ...
            <GlassCard><div className="p-3">
              ...
              <div className="text-xs">CUI {invoice.billing_snapshot?.cui} · {invoice.billing_snapshot?.reg_com}</div>
          ...
          {invoice.status === "paid" && (
            <div className="rounded bg-green-50 dark:bg-green-950/30 p-3 text-sm">
              ✓ Plătită {invoice.paid_at && new Date(invoice.paid_at).toLocaleDateString("ro-RO")} · Ref: {invoice.paid_ref}
            </div>
          )}
          {canConfirm && (
            <div className="border rounded p-3 space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2">
                <div><Label>Sumă încasată (RON)</Label><Input value={amount} onChange={(e) => setAmount(e.target.value)} /></div>
                <div><Label>Referință OP</Label><Input value={paidRef} onChange={(e) => setPaidRef(e.target.value)} placeholder="ex: nr. extras / OP" /></div>
                <div><Label>Data încasării</Label><Input type="date" value={paidAt} onChange={(e) => setPaidAt(e.target.value)} /></div>
              </div>
              <Button onClick={onConfirm} disabled={busy || !paidRef}>{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : "Confirmă & activează"}</Button>
            </div>
          )}
        </div>
        <DialogFooter><Button variant="outline" onClick={onClose}>Închide</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  );
}
...
export function RevenuePanel() {
  ...
  const load = useCallback(async () => {
    setLoading(true); setError(null);
    try { setData(await summary({})); } catch (e: any) { setError(e.message); }
    finally { setLoading(false); }
  }, [summary]);
  useEffect(() => { load(); }, [load]);

  const runTick = async () => {
    try { const r = await tick({}); toast.success(`Tick: gen ${r.result.generated} · overdue ${r.result.overdue} · downgraded ${r.result.downgraded}`); load(); }
    catch (e: any) { toast.error(e.message); }
  };

  if (loading) return <div className="p-8 flex justify-center"><Loader2 className="animate-spin" /></div>;
  if (error) return <div className="p-4 text-destructive">Eroare: {error} <Button size="sm" onClick={load}>Reîncearcă</Button></div>;
  if (!data) return null;
  ...
  return (
    <div className="space-y-4">
      ...
      <div><Button variant="outline" onClick={runTick}><RefreshCw className="h-4 w-4 mr-1" />Rulează billing-tick manual</Button></div>
    </div>
  );
}
...
export function BillingSettingsPanel({ isAdmin }: { isAdmin: boolean }) {
  ...
  useEffect(() => {
    (async () => {
      try { const r = await get({}); setVal(JSON.stringify(r.settings, null, 2)); }
  ...
  const save = async () => {
    setBusy(true);
    try { const parsed = JSON.parse(val); await upd({ data: { value: parsed } }); toast.success("Setări salvate"); }
    catch (e: any) { toast.error(e.message); }
    finally { setBusy(false); }
  };
  ...
  return (
    <div className="space-y-3">
      {!isAdmin && <div className="p-3 rounded bg-yellow-50 dark:bg-yellow-950/30 text-sm">Doar admin/super_admin poate modifica setările.</div>}
      <Textarea value={val} onChange={(e) => setVal(e.target.value)} rows={22} className="font-mono text-xs" disabled={!isAdmin} />
      <Button onClick={save} disabled={!isAdmin || busy}>{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : "Salvează (audit)"}</Button>
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2 border-b pb-2">
        <Button variant={tab === "invoices" ? "default" : "ghost"} size="sm" onClick={() => setTab("invoices")}>Facturi</Button>
        <Button variant={tab === "revenue" ? "default" : "ghost"} size="sm" onClick={() => setTab("revenue")}>Venituri</Button>
        <Button variant={tab === "settings" ? "default" : "ghost"} size="sm" onClick={() => setTab("settings")}>Setări facturare</Button>
      </div>
