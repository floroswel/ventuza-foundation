          ...
          <div className="flex items-end">
            <ReasonDialog
              ...
              onConfirm={async (reason) => {
                ...
                try {
                  const r = await start({
                    data: { targetUserId: uid, purpose: "support", justification: reason },
                  });
                  setDossier(r.dossier);
                  setImpId(r.impersonationId);
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-xl border border-border/60 bg-surface-elevated p-3 text-xs">
                <p className="mb-1 font-semibold">
                  {dossier.profile.display_name} ({dossier.profile.email})
                </p>
              ...
              <div className="rounded-xl border border-border/60 bg-surface-elevated p-3 text-xs">
                ...
                <p>
                  Ticket-uri deschise:{" "}
                  {(dossier.tickets ?? []).filter((t: any) => t.status === "open").length}
                </p>
          ...
          {authInfo && (
            <div className="mt-3 space-y-2">
              ...
              <div className="rounded-xl border border-border/60 bg-surface-elevated p-3 text-xs">
                ...
                <p>
                  Providers:{" "}
                  {authInfo.user?.identities?.map((i: any) => i.provider).join(", ") || "—"}
                </p>
                ...
                <div className="mt-3">
                  <p className="mb-1 font-semibold">
                    Push subscriptions ({authInfo.pushSubscriptions?.length ?? 0})
                  </p>
                      ...
                      >
                        ...
                        {!s.disabled_at && (
                          <ReasonDialog
                            ...
                            onConfirm={async (reason) => {
                              await revokeSub({
                                data: { userId: uid, subscriptionId: s.id, reason },
                              });
                              const a = await getAuth({ data: { userId: uid } });
                              setAuthInfo(a);
                            }}
...
function ImpersonationLogBlock() {
  ...
  const load = async () => {
    ...
    setErr(null);
    try {
      setRows(await list({ data: { limit: 200 } }));
    } catch (e: any) {
          ...
          {rows.map((r) => (
            <tr key={r.id} className="border-t border-border/50">
              ...
              <td className="px-2 py-1 text-center">
                {r.ended_at ? new Date(r.ended_at).toLocaleString("ro-RO") : "—"}
              </td>
            </tr>
          ))}
        </tbody>
