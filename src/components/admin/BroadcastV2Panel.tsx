...
export function BroadcastV2Panel() {
  ...
  const doPreview = async () => {
    setBusy(true);
    try {
      const r = await preview({ data: { filter: filter() } });
      setCount(r.count);
    } catch (e: any) {
  ...
  const doSend = async () => {
    if (title.length < 3 || body.length < 3) {
      toast.error("Titlu / mesaj prea scurt.");
      return;
    }
    if (count === null) {
      toast.error("Fă preview mai întâi.");
      return;
    }
    ...
    try {
      const r = await send({
        data: { title, body, link: link || undefined, channel, filter: filter() },
      });
      toast.success(`Trimis: ${r.delivered}/${r.target}`);
      loadCampaigns();
    } catch (e: any) {
      toast.error(e?.message ?? "Eroare");
    } finally {
  ...
  return (
    <div className="space-y-4">
            ...
            <div className="flex items-end">
              <label className="flex items-center gap-1.5 text-xs">
                <input
                  type="checkbox"
                  checked={dormant30d}
                  onChange={(e) => setDormant30d(e.target.checked)}
                />
                Inactivi &gt; 30 zile
              </label>
            </div>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          ...
          {count !== null && (
            <StatusBadge tone="approved">
              Estimare: {count.toLocaleString("ro-RO")} destinatari
            </StatusBadge>
          )}
      ...
      <GlassCard>
        {rows.length === 0 && (
          <p className="py-4 text-center text-xs text-muted-foreground">Fără campanii.</p>
        )}
        <table className="w-full text-xs">
          <thead className="text-[10px] uppercase text-muted-foreground">
            <tr>
              <th className="px-2 py-1 text-left">Când</th>
              <th>Titlu</th>
              <th>Status</th>
              <th>Target</th>
              <th>Delivered</th>
