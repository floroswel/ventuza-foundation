...
export function EnterpriseUsersPanel({ meId }: { meId: string }) {
  ...
  const [bulkAction, setBulkAction] = useState<"ban" | "unban" | "suspend" | "verify" | "unverify">(
    "suspend",
  );
  ...
  const exportCsv = () => {
    ...
    for (const r of rows) {
      ...
      const row = [
        ...
        JSON.stringify(r.travel_city ?? ""),
        r.verified ? "1" : "0",
        status,
        JSON.stringify((r.roles || []).join("|")),
        r.report_count ?? 0,
        r.risk_score ?? 0,
        r.xp ?? 0,
        r.level ?? 0,
      ...
      {loadingStats || !stats ? (
        ...
        <div className="grid grid-cols-2 gap-2 md:grid-cols-4 xl:grid-cols-7">
          ...
          <Kpi
            ...
            tone={stats.newDay > 0 ? "success" : "default"}
          />
          ...
          <Kpi
            label="Age pending"
            value={stats.pendingAge}
            icon={<UserX className="size-3" />}
            tone={stats.pendingAge > 0 ? "warn" : "default"}
          />
          <Kpi
            label="High risk"
      ...
      <GlassCard className="p-3">
        <div className="flex flex-wrap items-center gap-3 text-[11px]">
          ...
          <FilterGroup
            ...
            onChange={(v) => patch({ verified: v as any })}
            options={[
              ["any", "toate"],
              ["verified", "da"],
              ["unverified", "nu"],
          ...
          <FilterGroup
            ...
            onChange={(v) => patch({ createdSince: v as any })}
            options={[
              ["", "oricând"],
              ["24h", "24h"],
              ["7d", "7z"],
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            {activeFilterCount > 0 && (
              <button
                ...
                className="inline-flex items-center gap-1 rounded-full border border-[var(--admin-border)] px-2 py-1 text-[var(--admin-text-dim)] hover:border-[var(--admin-danger)]/40 hover:text-[var(--admin-danger)]"
              >
      ...
      {!error && rows.length > 0 && (
        <GlassCard className="p-0">
          <div className="flex items-center justify-between border-b border-[var(--admin-border)]/60 px-3 py-2 text-[11px] text-[var(--admin-text-dim)]">
            <span>
              <MonoNumber value={rows.length} /> rezultate{" "}
              {loading && <span className="opacity-60">(reîncarc…)</span>}
            </span>
            {bulk.count > 0 && (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-2">
                          <div className="relative size-8 shrink-0">
                            ...
                            {u.age_status === "verified" && (
                              <span
                                className="absolute -bottom-0.5 -right-0.5 size-2.5 rounded-full bg-[var(--admin-success)] ring-1 ring-black/40"
                                title="Vârstă verificată"
                              />
                            )}
                          </div>
                          <div className="min-w-0">
                            ...
                            <span
                              ...
                              title={u.id}
                            >
                              {u.id.slice(0, 8)}… · creat {rel(u.created_at)}
                            </span>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-2 py-2">
                        <div className="flex flex-wrap gap-1">
                          {u.roles.length === 0 && (
                            <span className="text-[10px] text-[var(--admin-text-faint)]">—</span>
                          )}
                          {u.roles.map((r) => (
                            <StatusBadge
                              key={r}
                      ...
                      <td className="px-2 py-2 text-[var(--admin-text-dim)]">
                        {u.travel_city ?? "—"}
                      </td>
                      <td className="px-2 py-2 text-right">
                        <MonoNumber
                          value={u.report_count ?? 0}
                          className={(u.report_count ?? 0) > 0 ? "text-[var(--admin-warn)]" : ""}
                        />
                      </td>
                      <td className="px-2 py-2 text-right">
                        <MonoNumber
                          value={u.risk_score ?? 0}
                          className={
                            (u.risk_score ?? 0) >= 70
                        ...
                        <div className="flex flex-wrap justify-end gap-1">
                          ...
                          {u.id !== meId && (
                            <button
                              ...
                              className="rounded-md bg-[var(--admin-danger-soft)] px-1.5 py-1 text-[var(--admin-danger)] hover:brightness-110"
                              title="Șterge cont"
                            >
