...
import {
  adminListBusinessApplications,
  adminDecideBusinessApplication,
  adminListPartners,
  adminSuspendPartner,
    ...
    <div className="space-y-4">
      <div className="flex gap-2 border-b border-border pb-2">
        <TabBtn
          ...
          icon={ListChecks}
          label="Coadă moderare"
        />
        <TabBtn
          active={tab === "apps"}
          onClick={() => setTab("apps")}
          icon={Building2}
          label="Aplicații parteneri"
        />
        <TabBtn
          active={tab === "partners"}
...
function BusinessApps({ canAdmin }: { canAdmin: boolean }) {
  ...
  const load = async () => {
    setBusy(true);
    try {
      setRows(await list({ data: { status, limit: 100 } }));
    } catch (e: any) {
  ...
  const act = async (id: string, decision: "approved" | "rejected") => {
    const notes =
      decision === "rejected" ? (prompt("Motivul respingerii (vizibil intern):") ?? "") : "";
    if (decision === "rejected" && !notes) return;
    try {
      await decide({ data: { id, decision, notes } });
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <button onClick={load} className="rounded-md border border-border px-2 py-1 text-xs">
      ...
      {busy ? (
        ...
        <p className="text-xs text-muted-foreground">Nicio aplicație.</p>
      ) : (
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="font-semibold">
                    {r.business_name}{" "}
                    <span className="ml-1 text-muted-foreground">· {r.business_type}</span>
                  </p>
                  ...
                  <p className="mt-1 text-muted-foreground">
                    Status: <b>{r.status}</b> · creată {new Date(r.created_at).toLocaleString()}
                  </p>
                  {r.review_notes && (
                    <p className="mt-1 italic text-muted-foreground">„{r.review_notes}"</p>
                  )}
                </div>
...
function PartnersList({ canAdmin }: { canAdmin: boolean }) {
  ...
  const load = async () => {
    setBusy(true);
    try {
      setRows(await list({ data: { filter, search: search || undefined, limit: 100 } }));
    } catch (e: any) {
      toast.error(e.message);
    } finally {
      setBusy(false);
  ...
  const onSuspend = async (id: string) => {
    const reason = prompt(
      "Motiv suspendare partener (vizibil intern, conținutul se ascunde instant):",
    );
    if (!reason) return;
    try {
      await suspend({ data: { user_id: id, reason } });
      toast.success("Suspendat — venues/oferte ascunse");
      load();
    } catch (e: any) {
      toast.error(e.message);
    }
  ...
  const onReinstate = async (id: string) => {
    ...
    try {
      await reinstate({ data: { user_id: id } });
  ...
  return (
    <div className="space-y-3">
      ...
      {busy ? (
        ...
        <p className="text-xs text-muted-foreground">Niciun partener.</p>
      ) : (
        <div className="space-y-2">
          {rows.map((p) => (
            <div key={p.id} className="rounded-lg border border-border bg-surface p-3 text-xs">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="text-muted-foreground">
                    Venues: <b>{p.stats.venues}</b> ({p.stats.published} publicate) · Oferte:{" "}
                    <b>{p.stats.offers}</b>
                  </p>
...
function ModerationQueue() {
  ...
  const [status, setStatus] = useState<
    "pending" | "changes_requested" | "rejected" | "approved" | "all"
  >("pending");
  const [rows, setRows] = useState<any[]>([]);
  const [busy, setBusy] = useState(false);
  ...
  const load = async () => {
    setBusy(true);
    try {
      setRows(await list({ data: { kind, status, limit: 150 } }));
    } catch (e: any) {
      toast.error(e.message);
    } finally {
      setBusy(false);
  ...
  return (
    <div className="space-y-3">
      ...
      {busy ? (
        ...
        <p className="text-xs text-muted-foreground">Coadă goală.</p>
      ) : (
        <div className="space-y-2">
          {rows.map((r) => {
...
function ModerationDetail({
  ...
  item: { kind: "venue" | "event" | "offer"; id: string };
  onClose: () => void;
}) {
  ...
  const act = async (decision: "approved" | "rejected" | "changes_requested") => {
    const reason =
      decision === "approved" ? undefined : (prompt(`Motiv (${decision}):`) ?? undefined);
  ...
  return (
    ...
    >
      <div
        className="max-h-[90dvh] w-full max-w-2xl overflow-auto rounded-xl bg-background p-5"
        ...
        {!data ? (
          ...
          <div className="space-y-4 text-sm">
            <header className="flex items-start justify-between gap-3">
              <div>
                <p className="text-xs uppercase tracking-wide text-muted-foreground">
                  {item.kind} · {data.moderation_status}
                </p>
            ...
            {(data.cover_url || data.image_url) && (
              <img
                src={data.cover_url || data.image_url}
                alt=""
            ...
            <dl className="grid grid-cols-2 gap-2 text-xs">
              {data.starts_at && (
                <Field k="Început" v={new Date(data.starts_at).toLocaleString()} />
              )}
              {data.ends_at && <Field k="Sfârșit" v={new Date(data.ends_at).toLocaleString()} />}
              {data.valid_from && (
                <Field k="Valabil de la" v={new Date(data.valid_from).toLocaleString()} />
              )}
              {data.valid_until && (
                <Field k="Valabil până la" v={new Date(data.valid_until).toLocaleString()} />
              )}
            ...
            {item.kind === "offer" && stats && (
              <p className="rounded-md bg-surface px-3 py-2 text-xs">
                Revendicări: <b>{stats.total}</b> · răscumpărate: <b>{stats.redeemed}</b> (fără
                identități utilizatori)
              </p>
            )}
              ...
              <div className="space-y-2 rounded-md border border-border p-3">
                <label className="block text-xs">
                  ...
                  <span className="text-[10px] text-muted-foreground">
                    Limitat anti-spam (max 10km, throttle notificări per partener).
                  </span>
                </label>
            ...
            <div className="rounded-md border border-yellow-500/40 bg-yellow-500/5 p-2 text-[11px] text-yellow-300/80">
              ...
              Verifică text și imagini pentru: escort/sex-work, conținut violent, minori, discurs
              incitator, spam. Aprobarea este singura poartă spre nearby — owner-ul NU poate publica
              singur.
            </div>
