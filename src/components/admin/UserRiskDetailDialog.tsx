        ...
        <div className="max-h-[calc(90vh-72px)] space-y-4 overflow-y-auto p-4">
          ...
          {error && (
            <div className="rounded-lg border border-red-500/40 bg-red-500/10 p-3 text-sm text-red-300">
              ...
              <button
                onClick={load}
                className="ml-2 rounded-full border border-red-500/40 px-2 py-0.5 text-xs"
              >
                Reîncearcă
              </button>
            </div>
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
                <Kpi
                  ...
                  value={
                    <span
                      className={`rounded border px-2 py-0.5 text-base ${scoreTone(p.risk_score ?? 0)}`}
                    >
                      {p.risk_score ?? 0}
                    </span>
                  }
                />
                <Kpi
                  label="Flag-uri totale"
                  value={flags.length}
                  icon={<Flag className="size-3" />}
                />
                <Kpi
                  label="Rapoarte primite"
                  value={detail.reports_received}
              ...
              <Section
                title="Compoziție scor (snapshot curent)"
              ...
              >
                ...
                {breakdown.length === 0 ? (
                  <div className="py-2 text-xs text-muted-foreground">
                    Niciun semnal numeric înregistrat încă.
                  </div>
                ) : (
                    ...
                    {breakdown.map(([k, v]) => {
                      ...
                      return (
                        <div key={k} className="flex items-center gap-2">
                          <div className="w-40 truncate text-[11px] font-medium uppercase tracking-wide text-muted-foreground">
                            {k}
                          </div>
                          ...
                          <div
                            className={`w-12 text-right font-mono text-xs ${positive ? "text-amber-300" : "text-emerald-300"}`}
                          >
                            {positive ? "+" : ""}
                            {v}
                          </div>
                  ...
                  <ol className="relative ml-2 space-y-3 border-l border-border/60 pl-4">
                    {flags.map((f) => (
                      <li key={f.id} className="relative">
                        <span
                          className={`absolute -left-[21px] mt-1.5 inline-block size-3 rounded-full ring-2 ring-background ${sevDot(f.severity)}`}
                        />
                        <div className="flex flex-wrap items-center gap-2 text-xs">
                          ...
                          <span
                            className={`rounded border px-1.5 py-0 text-[10px] ${scoreTone(f.severity)}`}
                          >
                            sev {f.severity}
                          </span>
                          {f.status === "open" ? (
