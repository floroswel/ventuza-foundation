...
export function SkeletonBlock({ h = 80, className = "" }: { h?: number; className?: string }) {
  return (
    <div
      className={`admin-shimmer rounded-2xl bg-surface-elevated/60 ${className}`}
      style={{ height: h }}
    />
  );
}
...
export function SkeletonTable({ rows = 6, cols = 4 }: { rows?: number; cols?: number }) {
  return (
    <div className="overflow-hidden rounded-2xl border border-border/60 bg-surface/40">
      <div
        className="grid gap-3 border-b border-border/60 px-4 py-3"
        style={{ gridTemplateColumns: `repeat(${cols}, minmax(0,1fr))` }}
      >
        {Array.from({ length: cols }).map((_, i) => (
          <SkeletonLine key={i} h={10} w="60%" />
        ))}
      </div>
      {Array.from({ length: rows }).map((_, r) => (
        <div
          key={r}
          className="grid gap-3 border-b border-border/40 px-4 py-3 last:border-0"
          style={{ gridTemplateColumns: `repeat(${cols}, minmax(0,1fr))` }}
        >
