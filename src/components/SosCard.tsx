...
export function SosCard() {
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from("profiles").select("sos_contacts").eq("id", user.id).maybeSingle()
      .then(({ data }) => {
  ...
  async function persist(next: Contact[]) {
    ...
    setSaving(true);
    setContacts(next);
    const { error } = await supabase.from("profiles").update({ sos_contacts: next }).eq("id", user.id);
    setSaving(false);
    if (error) toast.error(error.message);
  }

  function addContact() {
    if (!draft.name.trim()) { toast.error("Adaugă un nume."); return; }
    if (!draft.phone?.trim() && !draft.email?.trim()) { toast.error("Adaugă telefon sau email."); return; }
    if (contacts.length >= MAX_CONTACTS) { toast.error("Maxim 3 contacte."); return; }
  ...
  async function fireSOS() {
    ...
    try {
      ...
      toast.success(opened > 0 ? `SOS pregătit pentru ${opened} contact(e). Confirmă trimiterea.` : "SOS logat. Adaugă contacte pentru a notifica.");
    } catch (e) {
  ...
  return (
    <section className="rounded-2xl border border-destructive/40 bg-destructive/5 p-4">
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        La apăsare: trimite locația ta către contactele de încredere (până la 3). Doar pentru urgențe reale.
      </p>

      <ul className="mt-3 space-y-2">
        {contacts.map((c, i) => (
          <li key={i} className="flex items-center justify-between rounded-xl border border-border bg-background px-3 py-2 text-xs">
            <div className="min-w-0">
              <p className="truncate font-medium">{c.name}</p>
              <p className="truncate text-[11px] text-muted-foreground">{c.phone || c.email}</p>
            </div>
            <button onClick={() => removeContact(i)} className="ml-2 rounded-full p-1.5 text-muted-foreground hover:text-destructive">
      ...
      {contacts.length < MAX_CONTACTS && (
        <div className="mt-3 space-y-2 rounded-xl border border-dashed border-border bg-background/40 p-3">
          <input value={draft.name} onChange={(e) => setDraft({ ...draft, name: e.target.value })}
            placeholder="Nume (ex: Mama)" maxLength={40}
      ...
      <div className="mt-4 border-t border-destructive/30 pt-3">
        {!armed ? (
          ...
          <div className="space-y-2">
            <p className="text-center text-xs font-medium text-destructive">Confirmă: trimite SOS acum?</p>
            <div className="grid grid-cols-2 gap-2">
              <button onClick={() => setArmed(false)} className="rounded-full border border-border bg-background py-2 text-xs">
                Anulează
              </button>
              <button onClick={fireSOS} disabled={firing}
        ...
        <p className="mt-2 text-center text-[10px] text-muted-foreground">
          În caz de pericol imediat: <a href="tel:112" className="font-semibold text-destructive">sună 112</a>.
          {" "}Linie ACCEPT LGBTQ+: <a href="tel:+40213120884" className="font-medium underline">021 312 0884</a>.
        </p>
      </div>
    </section>
  );
