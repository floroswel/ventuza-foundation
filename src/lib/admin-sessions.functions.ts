...
async function logAudit(actorId: string, action: string, targetId: string, extra?: any, severity: "info" | "warning" | "critical" = "warning") {
  ...
  await (supabaseAdmin as any).from("admin_audit_log").insert({
    actor_id: actorId, action, target_table: "auth.users", target_id: targetId,
    after_data: extra ?? null, severity, ip, user_agent: ua,
  });
}
...
export const adminGetUserAuth = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [{ data: pushSubs }, { data: fps }] = await Promise.all([
      sa.from("push_subscriptions").select("id, platform, created_at, last_seen_at, disabled_at").eq("user_id", data.userId).order("last_seen_at", { ascending: false }),
      sa.from("device_fingerprints").select("id, first_seen_at, last_seen_at, seen_count").eq("user_id", data.userId).order("last_seen_at", { ascending: false }).limit(20),
    ]);
    return {
      user: {
        id: u.user?.id,
        ...
        identities: (u.user?.identities ?? []).map((i: any) => ({ provider: i.provider, created_at: i.created_at, last_sign_in_at: i.last_sign_in_at })),
      },
...
export const adminRevokeUserSessions = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error: banErr } = await sa.auth.admin.updateUserById(data.userId, { ban_duration: "1s" });
    ...
    await sa.from("push_subscriptions").update({ disabled_at: new Date().toISOString() })
      .eq("user_id", data.userId).is("disabled_at", null);
    await logAudit(context.userId, "session_revoke_all", data.userId, { reason: data.reason }, "warning");
    return { ok: true };
  });
...
export const adminRevokePushSub = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const sa = supabaseAdmin as any;
    await sa.from("push_subscriptions").update({ disabled_at: new Date().toISOString() })
      .eq("id", data.subscriptionId).eq("user_id", data.userId);
    await logAudit(context.userId, "push_sub_revoke", data.userId, { subscription_id: data.subscriptionId, reason: data.reason }, "info");
    return { ok: true };
  });
