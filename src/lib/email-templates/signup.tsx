...
export const SignupEmail = ({ siteName, siteUrl, recipient, confirmationUrl, locale }: Props) => {
  const t = getEmailStrings(locale)
  const lang = (locale as string) || 'ro'
  return (
    <Html lang={lang} dir="ltr">
      <Head />
      <Preview>{t.signup.preview(siteName)}</Preview>
      <Body style={main}>
        <Container style={container}>
          ...
          <Hr style={hr} />
          <Heading style={h1}>{t.signup.heading}</Heading>
          <Text style={text}>{t.signup.body(siteName)}</Text>
          <Text style={emailBadge}>{recipient}</Text>
          <Section style={buttonWrap}>
            <Button style={button} href={confirmationUrl}>{t.signup.cta}</Button>
          </Section>
          <Text style={smallText}>
            {t.common.fallbackLink}
            <br />
            <Link href={confirmationUrl} style={linkPlain}>{confirmationUrl}</Link>
          </Text>
          <Hr style={hr} />
          <Text style={footer}>{t.signup.ignore(siteName)}</Text>
          <Text style={footerBrand}>
            <Link href={siteUrl} style={footerLink}>{siteName}</Link> · {t.common.footerLocation}
          </Text>
        </Container>
      </Body>
    </Html>
...
const text = { fontSize: '15px', color: '#2a2a2a', lineHeight: '1.65', margin: '0 0 16px' }
const smallText = { fontSize: '12px', color: '#8a8578', lineHeight: '1.6', margin: '20px 0 0', wordBreak: 'break-all' as const }
const emailBadge = { display: 'inline-block', padding: '8px 14px', backgroundColor: '#f7f3ec', borderRadius: '6px', fontSize: '13px', color: '#0E0D0B', fontWeight: 500 as const, margin: '0 0 28px' }
