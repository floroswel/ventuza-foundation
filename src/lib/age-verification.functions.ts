...
export const startAgeVerification = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: hasConsent, error: consentErr } = await context.supabase.rpc(
      "has_active_consent",
      {
    ...
    if (consentErr) {
      ...
      return {
        ok: false as const,
        error: "consent_check",
        message: "Nu am putut verifica consimțământul.",
      };
    }
    if (hasConsent !== true) {
      return {
    ...
    if (!apiKey || !workflowId) {
      return {
        ...
        message: "Verificarea de vârstă nu este configurată.",
      };
    }
    ...
    try {
      ...
      return {
        ok: false as const,
        error: "network",
        message: "Nu am putut contacta serviciul de verificare. Încearcă mai târziu.",
      };
    }
    ...
    if (!res.ok) {
      ...
      if (/credits/i.test(detail)) {
        return {
          ...
          error: "no_credits",
          message:
            "Serviciul de verificare a vârstei este temporar indisponibil. Te rugăm încearcă mai târziu.",
        };
      }
      return {
        ok: false as const,
        error: "provider",
        message: detail || `Verificare indisponibilă (${res.status}).`,
      };
    }
    ...
    try {
      ...
      return {
        ok: false as const,
        error: "bad_response",
        message: "Răspuns invalid de la furnizor.",
      };
    }
    if (!json.url) {
    ...
    if (pendingError) {
      ...
      return {
        ok: false as const,
        error: "status_update",
        message: "Nu am putut actualiza statusul verificării. Încearcă din nou.",
      };
    }
