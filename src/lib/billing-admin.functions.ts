...
export const adminListInvoices = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.search)
      q = q.or(`payment_code.ilike.%${data.search}%,paid_ref.ilike.%${data.search}%`);
...
export const adminCancelInvoice = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d) =>
    z.object({ invoice_id: z.string().uuid(), reason: z.string().trim().min(3).max(500) }).parse(d),
  )
  .handler(async ({ data, context }) => {
    await requireStaff(context);
...
export const adminRevenueSummary = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [{ data: paidMonth }, { data: pending }, { data: overdue }, { data: subs }] =
      await Promise.all([
        (supabaseAdmin as any)
          .from("partner_invoices")
          .select("total_minor,paid_amount_minor")
          .eq("status", "paid")
    ...
    const sumMinor = (rows: any[] | null, key: string) =>
