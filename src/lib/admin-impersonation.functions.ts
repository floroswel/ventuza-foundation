...
export const startImpersonation = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // Log start
    const { data: log, error: logErr } = await sa
    ...
    const [prof, subs, roles, tickets, matches, convs, recentSwipes, recentReports, notif] =
      await Promise.all([
        sa
          .from("profiles")
          .select(
            "id, display_name, email, username, birthdate, age_status, verified, banned_at, suspended_until, report_count, last_active_at, created_at, city, country, plan_code, partner_role, partner_suspended_at",
          )
        ...
        sa
          ...
          .select("id, subject, status, priority, last_msg_at, created_at")
        ...
        sa
          ...
          .or(`user_a.eq.${data.targetUserId},user_b.eq.${data.targetUserId}`)
          .order("created_at", { ascending: false })
          .limit(20),
        sa
          .from("conversations")
          .select("id, participants, last_message_at, created_at")
          .contains("participants", [data.targetUserId])
          .order("last_message_at", { ascending: false })
          .limit(20),
        sa
          .from("swipes")
          .select("id, direction, created_at")
          .eq("swiper_id", data.targetUserId)
          .order("created_at", { ascending: false })
          .limit(50),
        sa
          .from("reports")
          .select("id, reason, status, created_at")
          .eq("reported_id", data.targetUserId)
          .order("created_at", { ascending: false })
          .limit(20),
        sa
          .from("notifications")
