...
async function selectAudience(sa: any, f: AudienceFilterT): Promise<string[]> {
  ...
  if (f.minAgeDays !== undefined) q = q.lte("created_at", new Date(Date.now() - f.minAgeDays * 86400000).toISOString());
  if (f.maxAgeDays !== undefined) q = q.gte("created_at", new Date(Date.now() - f.maxAgeDays * 86400000).toISOString());
  if (f.activeSince) q = q.gte("last_active_at", f.activeSince);
  if (f.hasNoActivity30d) q = q.lte("last_active_at", new Date(Date.now() - 30 * 86400000).toISOString());
  ...
  if (f.role) {
    ...
    if (f.role === "user") ids = ids.filter(i => !set.has(i)); else ids = ids.filter(i => set.has(i));
  }
  return ids;
}
...
export const broadcastSend = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: campaign, error: cErr } = await sa.from("broadcast_campaigns").insert({
    ...
    if (ids.length) {
      const rows = ids.map((uid) => ({
        user_id: uid, type: "admin_message" as const, title: data.title,
        body: data.body, link: data.link ?? null, actor_id: context.userId,
      }));
    ...
    await sa.from("broadcast_campaigns").update({
      status: "sent", delivered_count: delivered, finished_at: new Date().toISOString(),
    }).eq("id", campaign.id);

    await sa.from("admin_audit_log").insert({
      actor_id: context.userId, action: "broadcast_send",
      target_table: "broadcast_campaigns", target_id: campaign.id,
      after_data: { title: data.title, channel: data.channel, target: ids.length, delivered },
      severity: "info", ip, user_agent: ua,
    });
...
export const adminListCampaigns = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    await assertAdmin(context.supabase, context.userId);
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { data, error } = await (supabaseAdmin as any).from("broadcast_campaigns")
