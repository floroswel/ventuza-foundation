  ...
  useEffect(() => {
    ...
    (async () => {
      const { data: exp } = await supabase
        .from("experiments").select("variants, weights, status").eq("key", key).maybeSingle();
      ...
      const { data: existing } = await supabase
        .from("experiment_assignments").select("variant")
        .eq("experiment_key", key).eq("user_id", user.id).maybeSingle();

      let v = existing?.variant;
      if (!v) {
        v = pickVariant(variants, weights, `${key}:${user.id}`);
        await supabase.from("experiment_assignments")
...
export async function trackExperiment(key: string, event: string, value?: number) {
  ...
  const { data: assignment } = await supabase
    .from("experiment_assignments").select("variant")
    .eq("experiment_key", key).eq("user_id", u.user.id).maybeSingle();
  if (!assignment) return;
  await supabase.from("experiment_events").insert({
    experiment_key: key, variant: assignment.variant, user_id: u.user.id, event, value: value ?? null,
  });
}
