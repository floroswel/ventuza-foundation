...
CREATE OR REPLACE FUNCTION public.discover_profiles(
  ...
  _genders text[] DEFAULT NULL::text[],
  _tribes text[] DEFAULT NULL::text[],
  _looking_for text[] DEFAULT NULL::text[],
  _limit integer DEFAULT 30,
  _offset integer DEFAULT 0,
  _looking_now_only boolean DEFAULT false,
  _sort text DEFAULT 'smart'::text,
  _tab text DEFAULT 'all'::text,
  _orientation text[] DEFAULT NULL::text[],
  _body text[] DEFAULT NULL::text[],
  _position text[] DEFAULT NULL::text[],
  _min_height integer DEFAULT NULL::integer,
  _max_height integer DEFAULT NULL::integer,
...
RETURNS TABLE(
  ...
  score double precision, gender text[], orientation text[], looking_for text[],
...
-- 4) Invarianți de securitate actualizați
...
-- 5) Funcția apelată de pg_cron → endpointul HTTP de reconciliere Didit
...
BEGIN
  ...
  PERFORM net.http_post(
    url := 'https://ventuza-foundation.lovable.app/api/public/cron/didit-reconcile',
...
-- 6) Programare la fiecare 15 minute (idempotent)
DO $do$
BEGIN
  IF EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'didit-reconcile') THEN
    PERFORM cron.unschedule('didit-reconcile');
  END IF;
END
$do$;
SELECT cron.schedule('didit-reconcile', '*/15 * * * *', $$ SELECT public.cron_didit_reconcile(); $$);