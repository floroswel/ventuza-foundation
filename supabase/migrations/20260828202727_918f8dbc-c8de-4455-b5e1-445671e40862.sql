-- 1) Explorer: profilurile fără locație nu trebuie să apară în destinația aleasă.
...
DECLARE
  v_loc geography;
  v_viewer_explorer boolean := false;
...
BEGIN
  ...
  SELECT COALESCE(
           ...
           pv.location),
         (pv.travel_location IS NOT NULL AND pv.travel_until IS NOT NULL AND pv.travel_until > now())
    INTO v_loc, v_viewer_explorer
  ...
  filtered AS (
    SELECT * FROM base b
    WHERE (
        CASE WHEN v_viewer_explorer
             THEN b.dist_m IS NOT NULL AND b.dist_m <= _max_km * 1000
             ELSE b.dist_m IS NULL OR b.dist_m <= _max_km * 1000
        END
      )
...
-- 2) Anti privilege-escalation: coloanele privilegiate nu pot fi schimbate de owner.
...
BEGIN
  -- service_role / rulări server-side fără JWT de user: fără restricții.
  IF v_uid IS NULL THEN
    RETURN NEW;
  END IF;
  -- Staff-ul (moderator/admin/super_admin) poate modera prin căile lui.
  IF public.is_staff(v_uid) THEN
    RETURN NEW;
  END IF;
  IF v_uid IS DISTINCT FROM NEW.id THEN
    RETURN NEW; -- alte căi sunt oricum blocate de RLS
  END IF;

  -- Revert silențios pentru orice coloană privilegiată atinsă de user.
...
DROP TRIGGER IF EXISTS trg_prevent_profile_privileged_self_update ON public.profiles;
CREATE TRIGGER trg_prevent_profile_privileged_self_update
