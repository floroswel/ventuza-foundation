-- Reordonare Discover: online-first + fereastră presence 2 min (aliniat cu
-- isOnline() din client și heartbeat 45s). Semnătura rămâne aceeași.
...
DECLARE
  ...
  v_max_calls_per_hour constant int := 60;
...
BEGIN
  ...
  filtered AS (
    ...
    WHERE (b.dist_m IS NULL OR b.dist_m <= _max_km * 1000)
      ...
      -- 2 min: fereastra "online now" aliniată cu isOnline() + heartbeat 45s.
      -- hide_online=true → last_seen nu se actualizează (heartbeat skip) →
      -- utilizatorii invizibili sunt excluși natural.
      ...
      AND (
        _tab = 'all'
        OR (_tab = 'online'   AND b.last_seen IS NOT NULL AND b.last_seen > now() - interval '2 minutes')
  ...
  SELECT
    ...
    (
      ...
      + CASE WHEN f.last_seen IS NOT NULL AND f.last_seen > now() - interval '2 minutes' THEN 0.25 ELSE 0.0 END
  ...
  FROM filtered f
  ORDER BY
    -- Online-first: activi în ultimele 2 min sus. hide_online=true iese
    -- automat pentru că last_seen nu se împrospătează.
    (f.hide_online IS NOT TRUE AND f.last_seen IS NOT NULL
       AND f.last_seen > now() - interval '2 minutes') DESC,
