...
BEGIN
  IF NOT public.is_verification_staff(_uid) THEN
    RAISE WARNING '[verification_decide] forbidden actor=% request=%', _uid, p_request_id;
    RAISE EXCEPTION 'forbidden' USING ERRCODE='42501';
  END IF;
  IF p_decision NOT IN ('approve','reject','needs_second','appeal_required') THEN
    RAISE WARNING '[verification_decide] invalid_decision actor=% request=% decision=%', _uid, p_request_id, p_decision;
  ...
  IF p_reason_code IS NULL OR p_reason_code = '' THEN
    RAISE WARNING '[verification_decide] reason_code_required actor=% request=% decision=%', _uid, p_request_id, p_decision;
  ...
  IF NOT (p_reason_code = ANY (_allowed_reason_codes)) THEN
    RAISE WARNING '[verification_decide] invalid_reason_code actor=% request=% decision=% reason_code=% (allowed=%)',
      _uid, p_request_id, p_decision, p_reason_code, array_to_string(_allowed_reason_codes, ',');
  ...
  IF p_confidence IS NOT NULL AND p_confidence <> '' AND NOT (p_confidence = ANY (_allowed_confidence)) THEN
    RAISE WARNING '[verification_decide] invalid_confidence actor=% request=% confidence=% (allowed=%)',
      _uid, p_request_id, p_confidence, array_to_string(_allowed_confidence, ',');
  ...
  IF length(coalesce(p_reason,'')) < 3 THEN
    RAISE WARNING '[verification_decide] reason_required actor=% request=% length=%', _uid, p_request_id, length(coalesce(p_reason,''));
  ...
  IF _req.id IS NULL THEN
    RAISE WARNING '[verification_decide] not_found actor=% request=%', _uid, p_request_id;
  ...
  IF NOT _is_second AND _req.moderator_id IS DISTINCT FROM _uid THEN
    RAISE WARNING '[verification_decide] not_your_claim actor=% request=% moderator=%', _uid, p_request_id, _req.moderator_id;
  ...
  IF _is_second THEN
    IF p_decision NOT IN ('approve','reject') THEN
      RAISE WARNING '[verification_decide] second_review_binary actor=% request=% decision=%', _uid, p_request_id, p_decision;
