...
RETURNS TABLE(
  ...
  interests text[], prompts jsonb, distance_m double precision, last_seen timestamp with time zone,
  tribes text[], verified boolean, looking_now_until timestamp with time zone,
  looking_now_intent text, hide_age boolean, hide_distance boolean, hide_online boolean,
  score double precision, gender text[], orientation text[], looking_for text[], bio text,
  body_type text, height_cm integer, weight_kg integer, ethnicity text, "position" text,
  relationship_status text, boost_until timestamp with time zone, travel_city text,
...
-- Curăț log-urile vechi ca userii deja limitați să poată răsfoi imediat.
DELETE FROM public.rate_limit_log
 WHERE action = 'discover_profiles'
