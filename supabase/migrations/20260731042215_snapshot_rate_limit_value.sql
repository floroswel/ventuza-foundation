-- security_invariants_snapshot: raportează valoarea REALĂ a plafonului orar
-- (nu o constantă hardcodată în introspecție), ca testul de regresie să nu
-- devină fals-negativ când plafonul e ajustat printr-o migrare ulterioară.
...
BEGIN
  ...
  v_hour := NULLIF((regexp_match(COALESCE(v_discover, ''),
              'v_max_calls_per_hour\s+constant\s+int\s*:=\s*(\d+)'))[1], '')::int;
  ...
  v_result := jsonb_build_object(
    ...
    'discover_rate_limit_present', v_hour IS NOT NULL,
    'discover_max_calls_per_hour', v_hour,
...
REVOKE ALL ON FUNCTION public.security_invariants_snapshot() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.security_invariants_snapshot() TO service_role;
